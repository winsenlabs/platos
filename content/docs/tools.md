...
Exposure defaults to **Meta**. It is stored on the agent as `toolExposure` and applies to entity and skill tools only — runtime tools (`remember`, `spawn_job`, …) are always injected directly under either setting.
...
```json
{ "calls": [{ "tool": "gmail_send", "params": { "to": "user@example.com" } }] }
```
...
- BM25 is in-memory and per-process. A multi-replica installation needs each replica to keep the registry in sync via the WebSocket fan-out. Cold replicas pre-warm by replaying the registry from the gateway.
...
- Turning meta-tools off does not disarm memory or background work. `remember`, `recall`, `spawn_job` and the rest are runtime tools, always injected by name under both exposures.
...
- [Jobs](/docs/jobs): `spawn_job` is the runtime tool agents use to dispatch background work.
