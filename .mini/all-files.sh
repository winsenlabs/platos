...
pnpm exec vitest run src/files-rows.test.ts src/files-conformance.integration.test.ts src/files-constraints.integration.test.ts src/files-rules.integration.test.ts src/files-scope.integration.test.ts src/files-transaction.integration.test.ts src/files-statements.integration.test.ts --no-file-parallelism --testTimeout=180000 --hookTimeout=600000 2>&1 | tail -60
