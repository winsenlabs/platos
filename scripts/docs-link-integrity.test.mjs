...
test("the live corpora are enumerated with exact non-vacuous counts", () => {
  ...
  assert.deepEqual(result.stats, {
    ...
    //   +1 WIN-269 (M4.3) docs/audits/win-269-tool-lifecycle-reach.md, the
    //      rendered TOOL LIFECYCLE ORM register. It is GENERATED by the same
    //      shape of gate as its WIN-268 sibling above — `--write` emits it and
    //      `--check` fails when it drifts — and its only references are inline
