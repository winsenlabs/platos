...
test("generated ownership includes the generator's exact 165 outputs across 32 V1 projects", () => {
  const report = repositoryReport();
  // M2 INTEGRATION + MEMORY REBASE DELTA — 201 -> 165, as ONE reconciliation.
  // Adoption RELEASES placeholders, so this count only ever falls, and each
  // adopting slice releases placeholders from DISJOINT projects. The integrated
  // count is therefore the sum of every reduction, not the smallest branch pin:
  //
  ...
  //   169 -> 165  This branch adopts `memory` (context 8), releasing the same
  //               4 barrels every context adoption releases — domain/index.ts,
  ...
  //
  // THE SYSTEMIC SHAPE OF THIS DEFECT, recorded once here. Adoption ALWAYS
  // moves this number down by exactly four per context, and this canary is the
