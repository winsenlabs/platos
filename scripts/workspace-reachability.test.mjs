...
test("generated ownership includes the generator's exact 165 outputs across 32 V1 projects", () => {
  const report = repositoryReport();
  // WIN-256 CHANNELS DELTA — 169 -> 165, ONE merged delta on top of the M2
  // integration note below. Adopting `packages/contexts/channels` releases
  ...
  //
  // The branch this lands from (tejas/win-256-channels-context @ 4f6532a7)
  // pinned 174: it branched at 75ee484d, BEFORE `providers` (-4) and before
  // WIN-297's two apps (-9), so it never saw either release. 178 - 4 = 174 was
  // right on that base and is wrong here. 169 - 4 = 165 is the reconciled value
  // and it is DERIVED, not forced: the generator now reports
  // "9 project(s) adopted, 36 placeholder(s) released" against the 10 it
  // reported before, and 4 x 9 = 36.
  //
