...
test("generated ownership includes the generator's exact 165 outputs across 32 V1 projects", () => {
  ...
  // it pinned 182 - 9 = 173 and never saw the providers release; WIN-256's
  // providers tip pinned 178 and never saw the apps; WIN-256's tools tip pinned
  // 174 and never saw them either. No branch pin is correct here.
  // 201 - 19 - 4 - 9 - 4 = 165, which is 169 - 4, 174 - 9 and 173 - 4 - 4 alike.
  //
  // 169 -> 165. Adopting `tools` (context 7) releases its 4 placeholders —
  ...
  // SCAFFOLDING files plus the 68 placeholders of the 23 still-unadopted
  // projects. The scaffolding tier is untouched and stays byte-compared:
  ...
  // project's PLACEHOLDERS, not its scaffolding. That is what lets the
  // generator carry apps/core-api's new @nestjs runtime dependencies and its
  // two decorator compiler options, which scaffolding a hand edit could not
  // have added.
  //
  // THE ARITHMETIC IS THE POINT, because this is the canary four branches broke
  // this wave: 97 + 68 = 165, and 68 = 104 - 36 released (kernel 3 + six
  // contexts x 4 + core-api 8 + mcp-stdio 1). A number that does not decompose
  // that way means an adoption landed without its release, or a placeholder was
  // edited in place.
  //
