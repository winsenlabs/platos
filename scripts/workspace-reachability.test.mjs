...
test("generated ownership includes the generator's exact 201 outputs across 32 V1 projects", () => {
  const report = repositoryReport();
  assert.equal(report.generatedOwnership.ownedOutputCount, 201);
  ...
  assert.equal(report.generatedOwnership.generators[0].outputCount, 201);
...
test("an explicit generated header, not a generated-looking path, owns generated evidence", () => {
  ...
  const realHeader = fs
    .readFileSync(path.join(ROOT, "packages/kernel/src/index.ts"), "utf8")
    .split("\n", 1)[0];
