...
test("generated ownership includes the generator's exact 165 outputs across 32 V1 projects", () => {
  const report = repositoryReport();
  // M2 INTEGRATION + COST-MONITORING REBASE DELTA — 201 -> 165, as ONE
  // reconciliation. Adoption RELEASES placeholders, so this count only ever
  // falls, and each adopting slice releases placeholders from DISJOINT
  // projects. The integrated count is therefore the sum of every reduction,
  // not the smallest branch pin:
  //
  ...
  //   169 -> 165  This branch adopts `cost-monitoring` (context 13), releasing
  //               the same 4 barrels every context adoption releases.
  //
  // No parent pin was correct on its own: WIN-297 branched from WIN-256 at
  // 3ed8f3ce, BEFORE the providers commit, and pinned 173; WIN-256's tip pinned
  // 178 and never saw the apps; this branch pinned 174 (178 - 4) and never saw
  // either. 201 - 19 - 4 - 9 - 4 = 165, which is 169 - 4 and 174 - 9 alike.
  //
  ...
  // file was moved or a fifth placeholder was invented. The generator now owns
  // the same 97 scaffolding files plus the 68 placeholders of the 23
  // still-unadopted projects, and the project count is again unchanged at 32.
  // `node scripts/arch/gen-v1-skeleton.mjs --check` prints the same arithmetic
  // from the other side.
  //
  // THE SYSTEMIC SHAPE OF THIS DEFECT, recorded once here. This canary is the
  // FIRST step of the ci.yml typecheck job, so a branch that adopts a context
  // and leaves the constant alone is red before anything else runs.
