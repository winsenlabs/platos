...
test("§15: ownerDirectories grants exactly two directories, and only where declared", () => {
  ...
  // And `observability` is the THIRTEENTH. It is the entry that most needs
  // stating, because this owner also DRAINS two rows it is deliberately not the
  // writer of: `ownerDirectories("<kernel-outbox-adapter>")` below is a
  // different pair, and no part of this grant reaches it.
