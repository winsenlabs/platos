...
export function buildInventory(root = repositoryRoot) {
  ...
  const families = vocabularyFamilies(root) ?? [];
...
export function auditStreamContracts(root = repositoryRoot) {
  ...
  if (inventory === null) {
    ...
    if (JSON.stringify(inventory.families ?? []) !== JSON.stringify(moduleFamilies ?? [])) {
      problems.push(`S5 ${INVENTORY_PATH} carries a family list that is not the vocabulary's`);
    }
  }
