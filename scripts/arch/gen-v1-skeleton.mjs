...
export const ADAPTERS = [
  {
    ...
    additional: [
      ...
      // WIN-258 T5 adds the THIRTEENTH. `jobs` owns two canonical rows in that
...
// separately.
//
// 38 -> 40 (WIN-258 T5, a fifth time). `jobs` adds TWO canonical-store bindings,
// `JobsRepository` and `ApprovalsRepository`, over the two rows of ADR M0.3 §1
// row 15, and is the THIRTEENTH owner of the one PostgreSQL client. Two ports
...
// EXPECTED_ADAPTER_COUNT is deliberately unmoved a THIRTEENTH time, which is the
// point of pinning the two separately.
export const EXPECTED_ADAPTER_COUNT = 12;
export const EXPECTED_BINDING_COUNT = 40;
...
// separately on purpose.
//
// AND `packages/adapters/postgres-tenancy` -> `packages/contexts/jobs`. A
// THIRTEENTH owner edge, carrying that context's TWO canonical-store ports —
