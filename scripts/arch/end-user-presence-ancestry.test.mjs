...
test("the audit is non-vacuous: it reads real relations and real triggers", () => {
  ...
  const triggers = ancestryTriggerTables(repositoryRoot);
  assert.ok(triggers.size > 20, `expected the schema's ancestry triggers, saw ${triggers.size}`);
  assert.ok(triggers.has("Thread"));
});

test("the trigger replay honours DROP, so a removed trigger is not reported installed", () => {
  // The M4 migration drops and recreates Thread_ancestry against a different
  // function. A CREATE-only scan would be right here by luck; this proves the
  // replay, by dropping it in a later migration and never recreating it.
  withFixture((root) => {
    assert.ok(ancestryTriggerTables(root).has("Thread"));
    const later = join(root, MIGRATIONS_DIRECTORY, "99999999999999_drop_thread_ancestry");
    mkdirSync(later, { recursive: true });
    writeFileSync(join(later, "migration.sql"), 'DROP TRIGGER IF EXISTS "Thread_ancestry" ON "public"."Thread";\n');
    assert.ok(!ancestryTriggerTables(root).has("Thread"), "a dropped trigger was still reported installed");
  });
});

test("MUTATION: a presence relation whose model has no ancestry trigger goes RED", () => {
  // `identities` -> EndUserIdentity is the real hazard shape: it is a list
  // relation on EndUser and EndUserIdentity carries NO ancestry trigger (its
  ...
  withFixture((root) => {
    ...
    assert.ok(
      result.problems.some((problem) => problem.includes("EndUserIdentity") && problem.includes("NO _ancestry trigger")),
      `expected the unguarded relation to be reported, got: ${result.problems.join(" | ")}`,
    );
  });
});

test("MUTATION: dropping the Thread ancestry trigger goes RED", () => {
  withFixture((root) => {
    const later = join(root, MIGRATIONS_DIRECTORY, "99999999999999_drop_thread_ancestry");
    mkdirSync(later, { recursive: true });
    writeFileSync(join(later, "migration.sql"), 'DROP TRIGGER IF EXISTS "Thread_ancestry" ON "public"."Thread";\n');
    ...
    assert.ok(
      ...
      `expected the dropped trigger to be reported, got: ${result.problems.join(" | ")}`,
    );
  });
});
