...
test("the census is not vacuous — it reads the real suites", () => {
  ...
  assert.equal(live.totalFiles, 583);
...
test("the pinned rows sum to the pinned runtime total", () => {
  ...
  assert.equal(files, 583);
  assert.equal(files, /* WIN-268 stage 3 lifecycle suites, one per term */ (1 + 1) + /* WIN-268 P1 mint suite */ 1 + 503 + 6 + 12 + 5 + (2 + 1 + 1) + (1 + 2) + 7 + 3 + 2 + 3 + 1 + /* WIN-271 */ (5 + 1) + /* WIN-272 */ (3 + 1) + /* WIN-268 stage 2 tools/adapters */ 2 + /* 2026-09-15 notifier-email 2, identity-access 1, tenancy 2 */ 5 + /* WIN-271 D10 channel-discord */ 5 + /* WIN-271 (M4.5), D10, the remaining two providers: channel-whatsapp 5 and channel-telegram 4, both on ADAPTERS and neither on CONTEXTS */ 9);
});
...
test("the split the 2026-09-02 verification reproduced is pinned per package", () => {
  ...
  assert.equal(EXPECTED["packages/contexts/identity-access"].cases, 366, "231 at 3ed8f3ce, +25 contract suite, +28 end-user read, +34 session cookie, +6 server-side sign-out, +15 WIN-268 P1 bearer-credential mint, +16 WIN-268 stage 3 bearer-credential listing and revocation, +11 the 2026-09-15 magic-link facade and D3 re-recording");
  assert.equal(
...
test("the skills adoption is pinned, and moved nothing else", () => {
  ...
  const untouched = {
    ...
    "packages/contexts/identity-access": 366,
...
test("the WIN-257 identity-access contract suite is pinned at what vitest prints", () => {
  ...
  assert.equal(EXPECTED["packages/contexts/identity-access"].cases, 366);
  assert.ok(
