...
test("the census is not vacuous — it reads the real suites", () => {
  ...
  assert.equal(live.totalFiles, 509);
  ...
  // WIN-259 (M2.4) contributes 1 + 2 + 1: one to `packages/kernel`, two to
  // `packages/contexts/secrets`, and one to the postgres-tenancy adapter, where
  // it is the fence SPLIT rather than a new suite. Its SECOND pass contributes
  // 0 + 2 + 0: both reference suites are in `packages/contexts/secrets` and
  // neither the kernel nor the adapter moves, because a reference is a value
  // and implements no port.
  assert.equal(live.totalFiles, 88 + 14 + 20 + 16 + 28 + 21 + 15 + 15 + 25 + 19 + 15 + 31 + 4 + 2 + 15 + 29 + 1 + 2 + 4 + 3 + 4 + 7 + 5 + 4 + 4 + 6 + 6 + 6 + 1 + 6 + 6 + 9 + 7 + 8 + 6 + 7 + 6 + 7 + 7 + 5 + 6 + 3 + 6 + 3 + 1 + 1 + 2 + 1 + 2);
...
test("the split the 2026-09-02 verification reproduced is pinned per package", () => {
  ...
  assert.equal(EXPECTED["packages/contexts/secrets"].cases, 240, "162 before WIN-259, +13 write-only inputs, +11 denied-read evidence, +53 the secret reference, +1 its contract reachability");
...
test("the providers context is pinned at what vitest prints", () => {
  ...
  // WIN-259 (M2.4) adds a FOURTH term, +3: the three cases that close M23 in
  // `application/authorization.test.ts`, an existing file. The file term above
  // does not move for them, which is the drift a file count cannot see.
  assert.equal(EXPECTED["packages/contexts/providers"].cases, 283 + 63 + 29 + 3);
  ...
  // eleven ways cannot.
  assert.equal(EXPECTED_RUNTIME_TOTAL, 717 + 375 + 149 + 306 + 378 + 605 + 352 + 254 + 288 + 515 + 362 + 269 + 609 + 198 + 25 + 29 + 59 + 34 + 1 + 350 + 56 + 67 + 43 + 41 + 33 + 59 + 60 + 61 + 4 + 72 + 66 + 83 + 77 + 97 + 62 + 89 + 76 + 97 + 100 + 52 + 56 + 48 + 83 + 36 + 6 + 16 + 13 + 11 + 1 + 53 + 1 + 3 + 1);
});
...
test("the skills adoption is pinned, and moved nothing else", () => {
  ...
  const untouched = {
    ...
    "packages/contexts/secrets": 240,
    "packages/contexts/tenancy": 207,
    "packages/contexts/files": 134,
    "packages/contexts/providers": 378,
  };
  ...
  assert.equal(
    sum + 149 + 306 + 378 + 605 + 352 + 254 + 288 + 515 + 362 + 269 + 609 + 198 + 350 + 56 + 67 + 43 + 41 + 33 + 59 + 60 + 61 + 4 + 72 + 66 + 83 + 77 + 97 + 62 + 89 + 76 + 97 + 100 + 52 + 56 + 48 + 83 + 36 + 6 + 1 + 1,
    EXPECTED_RUNTIME_TOTAL,
  );
});
...
test("the conversations context is pinned at what vitest prints", () => {
  ...
  assert.equal(EXPECTED_RUNTIME_TOTAL, 5525 + 350 + 56 + 67 + 43 + 41 + 33 + 59 + 60 + 61 + 4 + 72 + 66 + 83 + 77 + 97 + 62 + 89 + 76 + 97 + 100 + 52 + 56 + 48 + 83 + 36 + 6 + 16 + 13 + 11 + 1 + 53 + 1 + 3 + 1);
});
...
test("the postgres-tenancy adapter is pinned at what vitest prints", () => {
  ...
  assert.equal(
    ...
    17 + 14 + 16 + 9 + 17 + 3 + 16 + 11 + 7 + 4 + 9 + 7 + 12 + 8 + 10 + 6 + 11 + 12 + 7 + 3 +
      ...
      1 +
      // WIN-259 (M2.4) SECOND PASS +1: the composite foreign key that refuses
      // an audit row naming a credential from ANOTHER environment, appended to
      // the same file. It adds no FILE either, so the file sum above is
      // unchanged and only this one moves.
      1,
  );
  // WIN-259 (M2.4) SECOND PASS 1484 -> 1485, in the INTEGRATION term and not
  // the runnable one: the composite foreign key that refuses an audit row
  // naming a credential from another environment, appended to the same
  // `secrets-rules.integration.test.ts`. It adds no FILE, so the 133 above is
  // unchanged and the integration case term goes 1055 -> 1056.
  assert.equal(EXPECTED["packages/adapters/postgres-tenancy"].cases, 1485);
  // 429 of the 1485 run in `pnpm test:v1-packages`; the other 1056 need a Docker
  ...
  assert.equal(EXPECTED_RUNTIME_TOTAL, 5875 + 56 + 67 + 43 + 41 + 33 + 59 + 60 + 61 + 4 + 72 + 66 + 83 + 77 + 97 + 62 + 89 + 76 + 97 + 100 + 52 + 56 + 48 + 83 + 36 + 6 + 16 + 13 + 11 + 1 + 53 + 1 + 3 + 1);
  assert.equal(
    EXPECTED_RUNTIME_TOTAL,
    Object.values(EXPECTED).reduce((total, row) => total + row.cases, 0)
  );
