...
test("the census is not vacuous — it reads the real suites", () => {
  ...
  //  +11 WIN-259 (M2.4). SIX in the new `packages/adapters/keyring-envelope`
  //      row, ONE (`secrets-key-version.integration.test.ts`) in
  ...
  //      and `application/migrate-legacy-envelope.test.ts`. 503 + 11 = 514.
  //
  //      FOUR OF THE ELEVEN ARE THE LEGACY-ENVELOPE MIGRATION'S, and they come
  //      in a 2/2 split across the two packages because the deliverable is two
  //      halves that may not meet: the adapter owns the bytes and the context
  //      owns the rule and the control flow.
  assert.equal(live.totalFiles, 514);
  ...
  // WIN-259 (M2.4) appends `+ 6 + 1 + 1 + 3`: the six suites of the new
  // `packages/adapters/keyring-envelope` row, the one
  // `secrets-key-version.integration.test.ts` on the postgres row, the one
  ...
  assert.equal(live.totalFiles, 88 + 14 + 20 + 16 + 28 + 21 + 15 + 15 + 25 + 19 + 15 + 31 + 4 + 2 + 15 + 29 + 1 + 2 + 4 + 3 + 4 + 7 + 5 + 4 + 4 + 6 + 6 + 6 + 1 + 6 + 6 + 9 + 7 + 8 + 6 + 7 + 6 + 7 + 7 + 5 + 6 + 3 + 6 + 3 + 1 + 6 + 1 + 1 + 3);
...
test("the pinned rows sum to the pinned runtime total", () => {
  ...
  //
  // WIN-259 (M2.4) 503 -> 514 and 1483 -> 1488 on that row's own count; the
  // integration term goes 1054 -> 1059 over 109 -> 110 files and the runnable
  // term is unmoved at 429, because the tranche's one addition here carries
  // `.integration.` in its name. Its other 130 cases are in three OTHER rows,
  // and every one of them runs on a laptop.
  assert.equal(files, 514);
});
...
test("the providers context is pinned at what vitest prints", () => {
  ...
  // eleven ways cannot.
  assert.equal(EXPECTED_RUNTIME_TOTAL, 717 + 375 + 149 + 306 + 378 + 605 + 352 + 254 + 288 + 515 + 362 + 269 + 609 + 198 + 25 + 29 + 59 + 34 + 1 + 350 + 56 + 67 + 43 + 41 + 33 + 59 + 60 + 61 + 4 + 72 + 66 + 83 + 77 + 97 + 62 + 89 + 76 + 97 + 100 + 52 + 56 + 48 + 83 + 36 + 6 + 71 + 5 + 8 + 51);
});
...
test("the skills adoption is pinned, and moved nothing else", () => {
  ...
  assert.equal(
    ...
    sum + 149 + 306 + 378 + 605 + 352 + 254 + 288 + 515 + 362 + 269 + 609 + 198 + 350 + 56 + 67 + 43 + 41 + 33 + 59 + 60 + 61 + 4 + 72 + 66 + 83 + 77 + 97 + 62 + 89 + 76 + 97 + 100 + 52 + 56 + 48 + 83 + 36 + 6 + 71 + 5,
    EXPECTED_RUNTIME_TOTAL,
  );
});
...
test("the conversations context is pinned at what vitest prints", () => {
  ...
  assert.equal(EXPECTED_RUNTIME_TOTAL, 5525 + 350 + 56 + 67 + 43 + 41 + 33 + 59 + 60 + 61 + 4 + 72 + 66 + 83 + 77 + 97 + 62 + 89 + 76 + 97 + 100 + 52 + 56 + 48 + 83 + 36 + 6 + 71 + 5 + 8 + 51);
});
...
test("the postgres-tenancy adapter is pinned at what vitest prints", () => {
  ...
  assert.equal(
    ...
    // WIN-259 (M2.4) adds the trailing 1: `secrets-key-version.integration.test.ts`.
    2 + 2 + 1 + 6 + 1 + 4 + 4 + 6 + 6 + 6 + 1 + 6 + 6 + 9 + 7 + 8 + 6 + 7 + 6 + 7 + 7 + 5 + 6 + 3 + 6 + 3 + 1 + 1,
  );
  assert.equal(
    ...
    17 + 14 + 16 + 9 + 17 + 3 + 16 + 11 + 7 + 4 + 9 + 7 + 12 + 8 + 10 + 6 + 11 + 12 + 7 + 3 +
      ...
      11 + 6 + 9 + 9 + 1 +
      6 +
      // WIN-259 (M2.4). Five cases in one file, `secrets-key-version.integration.test.ts`:
      // two on the byte round trip, three on the rotation/re-encryption row pair.
      5,
  );
  assert.equal(EXPECTED["packages/adapters/postgres-tenancy"].cases, 1488);
  // 429 of the 1488 run in `pnpm test:v1-packages`; the other 1059 need a Docker
  // daemon and run in the `postgres-tenancy-repository` CI job. WIN-259's five
  // are all on the container side: the file's name carries `.integration.`,
  ...
  assert.equal(EXPECTED_RUNTIME_TOTAL, 5875 + 56 + 67 + 43 + 41 + 33 + 59 + 60 + 61 + 4 + 72 + 66 + 83 + 77 + 97 + 62 + 89 + 76 + 97 + 100 + 52 + 56 + 48 + 83 + 36 + 6 + 71 + 5 + 8 + 51);
  assert.equal(
    EXPECTED_RUNTIME_TOTAL,
    Object.values(EXPECTED).reduce((total, row) => total + row.cases, 0)
  );
