...
test("the census is not vacuous — it reads the real suites", () => {
  ...
  // No context row moves, for the fourth time. 410 + 6 = 416.
...
test("the pinned rows sum to the pinned runtime total", () => {
  ...
  // is none of them.
  //
  // `governance` ADDS 45 MORE over 5 files (1 + 12 + 10 + 15 + 7): 265 + 45 =
  // 310, over 36 files. Its sixth suite, `governance-rows.test.ts`, is NOT among
  // them — it is 21 pure cases over the mapping branches a container suite
  // cannot reach, and it runs in the ordinary package test script.
...
test("the providers context is pinned at what vitest prints", () => {
  ...
  assert.equal(EXPECTED_RUNTIME_TOTAL, 717 + 375 + 149 + 306 + 378 + 605 + 352 + 254 + 288 + 515 + 362 + 269 + 609 + 198 + 25 + 29 + 59 + 34 + 1 + 350 + 56 + 67 + 43 + 41 + 33 + 59 + 60 + 61 + 4 + 66);
});
...
test("the skills adoption is pinned, and moved nothing else", () => {
  ...
  assert.equal(
    sum + 149 + 306 + 378 + 605 + 352 + 254 + 288 + 515 + 362 + 269 + 609 + 198 + 350 + 56 + 67 + 43 + 41 + 33 + 59 + 60 + 61 + 4 + 66,
    EXPECTED_RUNTIME_TOTAL,
  );
});
...
test("the conversations context is pinned at what vitest prints", () => {
  ...
  assert.equal(EXPECTED_RUNTIME_TOTAL, 5525 + 350 + 56 + 67 + 43 + 41 + 33 + 59 + 60 + 61 + 4 + 66);
});
...
test("the postgres-tenancy adapter is pinned at what vitest prints", () => {
  ...
  // 123 + 43 + 33 + 59 + 60 + 61 + 4 + 66 = 449 cases. No branch's own row is
  ...
  assert.equal(EXPECTED["packages/adapters/postgres-tenancy"].cases, 449);
  ...
  assert.equal(EXPECTED_RUNTIME_TOTAL, 5875 + 56 + 67 + 43 + 41 + 33 + 59 + 60 + 61 + 4 + 66);
  assert.equal(
    EXPECTED_RUNTIME_TOTAL,
    Object.values(EXPECTED).reduce((total, row) => total + row.cases, 0)
  );
