...
test("the census is not vacuous — it reads the real suites", () => {
  ...
  // for the eighth time.
  //
  // AND `files` ADDS SEVEN MORE to the same row: six real-PostgreSQL and one
  // pure (`files-rows.test.ts`), taking the tree to 459 + 7 = 466. No CONTEXT
  // row moves for it either — the port it implements already existed, and
  // widening that port's entry point to publish the names its own signatures use
  // adds no case.
...
test("the providers context is pinned at what vitest prints", () => {
  ...
  assert.equal(EXPECTED_RUNTIME_TOTAL, 717 + 375 + 149 + 306 + 378 + 605 + 352 + 254 + 288 + 515 + 362 + 269 + 609 + 198 + 25 + 29 + 59 + 34 + 1 + 350 + 56 + 67 + 43 + 41 + 33 + 59 + 60 + 61 + 4 + 72 + 66 + 83 + 77 + 97 + 62 + 89 + 100);
});
...
test("the skills adoption is pinned, and moved nothing else", () => {
  ...
  // and the LAST 89 a sixth, for `memory`, whose two ports already existed and
  // whose 378-case context row does not move either.
  assert.equal(
    sum + 149 + 306 + 378 + 605 + 352 + 254 + 288 + 515 + 362 + 269 + 609 + 198 + 350 + 56 + 67 + 43 + 41 + 33 + 59 + 60 + 61 + 4 + 72 + 66 + 83 + 77 + 97 + 62 + 89 + 100,
    EXPECTED_RUNTIME_TOTAL,
  );
});
...
test("the conversations context is pinned at what vitest prints", () => {
  ...
  assert.equal(EXPECTED_RUNTIME_TOTAL, 5525 + 350 + 56 + 67 + 43 + 41 + 33 + 59 + 60 + 61 + 4 + 72 + 66 + 83 + 77 + 97 + 62 + 89 + 100);
});
...
test("the postgres-tenancy adapter is pinned at what vitest prints", () => {
  ...
  //
  // ALL FIVE tranches move THIS row, so it carries every tail:
  // 11 + 5 + 4 + 6 + 6 + 6 + 1 + 9 + 7 + 8 + 6 + 7 = 76 files and
  // 123 + 43 + 33 + 59 + 60 + 61 + 4 + 83 + 77 + 97 + 62 + 89 + 100 = 791 cases. No
  // branch's own row is the merged row, and any one taken alone would drop the
  // others' suites out of a census whose whole purpose is to see every case.
  assert.equal(
  ...
  // WIN-258 T5 (`files`): 20 + 2 + 15 + 16 + 27 + 9 + 6 = 95 over seven files —
  // the mapping suite, the differential and its negative control, the column
  // constraints, the five rules that live only in the migrations, one case per
  // clause of every scoped read, the failure injection with the three scope
  // refusals, and the measured statement counts.
  //
  // THE 27 IN THE SCOPE SUITE IS THE NUMBER TO READ, and it exists for the
  // reason the memory tranche's 5 does: the differential asks its cross-tenant
  // questions with EVERY id wrong at once, so a read that had dropped one clause
  // would still answer correctly and the mutation over that clause would have
  // survived. Thirty-one cases each wrong in ONE id is what closes them — and
  // FOUR of the thirty-one were added AFTER the first sweep, which left seven
  // mutants alive because the cross-tenant cases named a chain that differs in
  // all three ids at once.
  assert.equal(EXPECTED["packages/adapters/postgres-tenancy"].cases, 1029);
  // 266 of the 929 run in `pnpm test:v1-packages`; the other 663 need a Docker
  // daemon and run in the `postgres-tenancy-repository` CI job. A pin that
  // counted only the runnable 266 would go green if the integration suites were
  // deleted, which is the one change this row exists to make visible.
  assert.equal(EXPECTED_RUNTIME_TOTAL, 5875 + 56 + 67 + 43 + 41 + 33 + 59 + 60 + 61 + 4 + 72 + 66 + 83 + 77 + 97 + 62 + 89 + 100);
  assert.equal(
    EXPECTED_RUNTIME_TOTAL,
    Object.values(EXPECTED).reduce((total, row) => total + row.cases, 0)
  );
