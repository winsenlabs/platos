...
test("the skills adoption is pinned, and moved nothing else", () => {
  ...
  assert.equal(
    ...
    // WIN-269 (M4.3) moves the `tools` term 400 -> 402 IN PLACE, and it is an
    // in-place move for the reason the two before it were: the term is the whole
...
test("the tools context is pinned at what vitest prints", () => {
  ...
  // A FIFTH WAVE, AND IT MOVES NO FILE AGAIN (WIN-269, M4.3). 400 -> 402: the two
  // operator-gate cases for `ToolsContract.recordToolHealth`, written out one by
  // one beside their twenty-eight siblings because that file's own note says a
  // loop would declare the same assertions in four lines and this census could
  // not count them. `application/record-tool-health.ts` is a NEW FILE that adds
  // NO case here, and that is the honest reading rather than an omission: its
  // behaviour — the fold that leaves the call counters alone, the forged grant,
  // the genuine grant for another project, the unknown tool name it skips and
  // reports — is proved against a REAL PostgreSQL in
  ...
  // none.
  // 325 + 29 + 6 + 2 + 4 + 8 + 26 + 2 = 402.
  assert.equal(EXPECTED["packages/contexts/tools"].files, 21);
  assert.equal(EXPECTED["packages/contexts/tools"].cases, 402);
  assert.equal(EXPECTED["packages/contexts/tools"].cases, 325 + 29 + 6 + 2 + 4 + 8 + 26 + 2);
  assert.equal(EXPECTED_RUNTIME_TOTAL, /* WIN-268 stage 3 identity-access lifecycle rules, 14 written + 2 the mutation sweep forced */ 16 + /* WIN-268 stage 3 postgres-tenancy lifecycle over a real database, 10 written + 2 the sweep forced */ 12 + /* WIN-269 tools transport admission */ 4 + /* WIN-268 P1 mint suite */ 15 + 717 + 375 + 149 + 306 + 378 + 605 + 352 + 254 + 288 + 515 + 362 + 269 + 609 + 198 + 25 + 29 + 59 + 34 + 1 + 350 + 56 + 67 + 43 + 41 + 33 + 59 + 60 + 61 + 4 + 72 + 66 + 83 + 77 + 97 + 62 + 89 + 76 + 97 + 100 + 52 + 56 + 48 + 83 + 36 + 6 + 16 + 13 + 11 + 1 + 53 + 1 + 3 + 1 + 71 + 12 + 8 + 53 + 5 + 8 + 5 + 9 + 40 + 25 + 69 + 8 + 22 + 27 + 30 + 136 + 26 + 19 + 13 + 28 + 11 + 1 + 6 + /* WIN-271 channels delivery-disposition rule */ 5 + /* WIN-271 channel-slack adoption */ 61 + /* WIN-272 kernel stream envelope */ 36 + /* WIN-272 redis-streams adoption */ 46 + /* WIN-268 stage 2 tools/adapters: the digest's known-answer vectors 8 and the real-socket dispatch suite 26 */ 34 + /* WIN-269 (M4.3): the operator-gate pair for ToolsContract.recordToolHealth, in a file that already existed. The lane's twenty-seven real-database cases are in apps/core-api, which is outside PACKAGE_ROOTS. */ 2);
});
