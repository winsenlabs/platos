...
test("the census is not vacuous — it reads the real suites", () => {
  ...
  // for the eighth time.
  //
  // AND `observability` ADDS FIVE MORE to the same row a fifth time: four
  // real-PostgreSQL and one pure (`observability-rows.test.ts`). No CONTEXT row
  // moves for it either — the port already existed, and widening a port entry
  // point in place is not a new file. 459 + 5 = 464.
  assert.equal(live.totalFiles, 464);
  ...
  assert.equal(live.totalFiles, 93 + 14 + 20 + 16 + 28 + 21 + 15 + 15 + 25 + 19 + 15 + 31 + 4 + 2 + 15 + 29 + 1 + 2 + 4 + 3 + 4 + 7 + 5 + 4 + 4 + 6 + 6 + 6 + 1 + 6 + 6 + 9 + 7 + 8 + 6 + 7);
...
test("the pinned rows sum to the pinned runtime total", () => {
  ...
  // 118 + 25 + 21 + 18 + 17 + 24 + 19 + 24 + 15 = 281 while the integration term
  // goes 265 + 47 + 45 + 65 + 60 + 73 + 43 + 65 + 37 = 700, and 281 + 700 = 981
  // is the row's whole case count. `observability-rows.test.ts` (15 cases) is
  // the ninth pure suite, and it is the only one of its five that can reach a
  // snapshot column this binary cannot read.
  assert.equal(files, 464);
});
...
test("the providers context is pinned at what vitest prints", () => {
  ...
  assert.equal(EXPECTED_RUNTIME_TOTAL, 717 + 375 + 149 + 306 + 378 + 605 + 352 + 254 + 288 + 515 + 362 + 269 + 609 + 198 + 25 + 29 + 59 + 34 + 1 + 350 + 56 + 67 + 43 + 41 + 33 + 59 + 60 + 61 + 4 + 72 + 66 + 83 + 77 + 97 + 62 + 89 + 52);
});
...
test("the skills adoption is pinned, and moved nothing else", () => {
  ...
  // 378-case context row does not move either, and the LAST 52 a seventh, for
  // `observability`, whose ONE port already existed and whose context row is
  // untouched for the same reason again.
  assert.equal(
    sum + 149 + 306 + 378 + 605 + 352 + 254 + 288 + 515 + 362 + 269 + 609 + 198 + 350 + 56 + 67 + 43 + 41 + 33 + 59 + 60 + 61 + 4 + 72 + 66 + 83 + 77 + 97 + 62 + 89 + 52,
    EXPECTED_RUNTIME_TOTAL,
  );
});
...
test("the conversations context is pinned at what vitest prints", () => {
  ...
  assert.equal(EXPECTED_RUNTIME_TOTAL, 5525 + 350 + 56 + 67 + 43 + 41 + 33 + 59 + 60 + 61 + 4 + 72 + 66 + 83 + 77 + 97 + 62 + 89 + 52);
});
...
test("the postgres-tenancy adapter is pinned at what vitest prints", () => {
  ...
  //
  // ALL FIVE tranches move THIS row, so it carries every tail:
  // 11 + 5 + 4 + 6 + 6 + 6 + 1 + 9 + 7 + 8 + 6 + 7 + 5 = 81 files and
  // 123 + 43 + 33 + 59 + 60 + 61 + 4 + 83 + 77 + 97 + 62 + 89 + 52 = 843 cases.
  ...
  //
  // `observability` IS THE LAST TAIL AND THE SMALLEST STORE WITH THE LARGEST
  // EVIDENCE RATIO: 5 files and 52 cases for FOUR port methods over ONE table,
  // because `AdminAudit` is APPEND-ONLY IN THE DATABASE and one of those four
  // cannot be honoured at all.
  assert.equal(
    EXPECTED["packages/adapters/postgres-tenancy"].files,
    2 + 2 + 1 + 6 + 1 + 4 + 4 + 6 + 6 + 6 + 1 + 6 + 6 + 9 + 7 + 8 + 6 + 7 + 5,
  );
  assert.equal(
  ...
  assert.equal(EXPECTED["packages/adapters/postgres-tenancy"].cases, 981);
  ...
  assert.equal(EXPECTED_RUNTIME_TOTAL, 5875 + 56 + 67 + 43 + 41 + 33 + 59 + 60 + 61 + 4 + 72 + 66 + 83 + 77 + 97 + 62 + 89 + 52);
  assert.equal(
    EXPECTED_RUNTIME_TOTAL,
    Object.values(EXPECTED).reduce((total, row) => total + row.cases, 0)
  );
