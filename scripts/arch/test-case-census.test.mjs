...
test("the census is not vacuous — it reads the real suites", () => {
  ...
  //
  // WIN-258 TRANCHE 2 adds +7 suites in the postgres-tenancy adapter row, which
  // was already real at 4. 371 + 7 = 378. WIN-258 TRANCHE 3 adds the last term,
  // +5 to that same row — tenancy's other five ports, one pure suite and four
  // real-PostgreSQL ones. 378 + 5 = 383.
  assert.equal(live.totalFiles, 383);
  ...
  assert.equal(live.totalFiles, 88 + 14 + 20 + 16 + 28 + 21 + 15 + 15 + 25 + 19 + 15 + 31 + 4 + 2 + 15 + 29 + 1 + 2 + 4 + 3 + 4 + 7 + 5);
...
test("the pinned rows sum to the pinned runtime total", () => {
  ...
  //
  // WIN-258 TRANCHE 3 adds the last term, +5 to that same already-real row: one
  // pure suite for the invitation token issuer and FOUR real-PostgreSQL ones —
  // whether the locks block, the shared conformance scenario over the five
  // ports, the scope refusals with failure injection, and the statement counts.
  // 378 + 5 = 383.
  assert.equal(files, 383);
});
...
test("the providers context is pinned at what vitest prints", () => {
  ...
  assert.equal(EXPECTED_RUNTIME_TOTAL, 717 + 375 + 149 + 306 + 378 + 605 + 352 + 254 + 288 + 515 + 362 + 269 + 609 + 198 + 25 + 29 + 59 + 34 + 1 + 350 + 56 + 67 + 42);
});
...
test("the skills adoption is pinned, and moved nothing else", () => {
  ...
  // tranche 2's identity-access half of the SAME row.
  assert.equal(
    sum + 149 + 306 + 378 + 605 + 352 + 254 + 288 + 515 + 362 + 269 + 609 + 198 + 350 + 56 + 67 + 42,
    EXPECTED_RUNTIME_TOTAL,
  );
});
...
test("the conversations context is pinned at what vitest prints", () => {
  ...
  assert.equal(EXPECTED_RUNTIME_TOTAL, 5525 + 350 + 56 + 67 + 42);
});
...
test("the postgres-tenancy adapter is pinned at what vitest prints", () => {
  ...
  //
  // WIN-258 TRANCHE 3 adds FIVE more suites and 42 more cases to the same row —
  ...
  // plus the five cases it cannot reach (7), the three scope refusals on all
  // five scoped methods with failure injection and the OperatorSession database
  // rules (10), and the measured statement counts (6). 11 + 5 = 16 files,
  // 123 + 42 = 165.
  assert.equal(EXPECTED["packages/adapters/postgres-tenancy"].files, 2 + 2 + 1 + 6 + 1 + 4);
  assert.equal(
    EXPECTED["packages/adapters/postgres-tenancy"].cases,
    17 + 14 + 16 + 9 + 17 + 3 + 16 + 11 + 7 + 4 + 9 + 7 + 12 + 7 + 10 + 6,
  );
  assert.equal(EXPECTED["packages/adapters/postgres-tenancy"].cases, 165);
  // 55 of the 165 run in `pnpm test:v1-packages`; the other 110 need a Docker
  // daemon and run in the `postgres-tenancy-repository` CI job. A pin that
  // counted only the runnable 55 would go green if the integration suites were
  // deleted, which is the one change this row exists to make visible.
  assert.equal(EXPECTED_RUNTIME_TOTAL, 5875 + 56 + 67 + 42);
  assert.equal(
    EXPECTED_RUNTIME_TOTAL,
    Object.values(EXPECTED).reduce((total, row) => total + row.cases, 0)
  );
  // No other adapter row moved: ten of the twelve are still zero, and
  // model-router-providers is untouched at 198.
  assert.equal(EXPECTED["packages/adapters/model-router-providers"].cases, 198);
  const zeroAdapters = Object.entries(EXPECTED).filter(
    ([name, row]) => name.startsWith("packages/adapters/") && row.cases === 0
  );
  assert.equal(zeroAdapters.length, 10);
});
