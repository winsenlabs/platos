...
test("the census is not vacuous — it reads the real suites", () => {
  ...
  //
  // WIN-258 TRANCHE 5 adds ONE more term, +6, to that same postgres-tenancy row
  // — the `agents` canonical store. Two of the six have no database in them
  // (`agents-guards.test.ts`, `agents-rows.test.ts`) and four are
  // real-PostgreSQL. 391 + 6 = 397.
...
test("the pinned rows sum to the pinned runtime total", () => {
  ...
  // all.
  //
  // 378 + 5 + 4 + 4 = 391, and the integration cases the
  // `postgres-tenancy-repository` CI job runs and `pnpm test:v1-packages` does
  // not are the ninety above plus tranche 3's 36 and tranche 4's 33.
  //
  // WIN-258 TRANCHE 5 adds the last term: +6 to that same postgres-tenancy row,
  // FOUR of them real-PostgreSQL suites for the seven `agents` rows and two with
  // no database in them. 391 + 6 = 397.
  //
  // COUNTED RATHER THAN CARRIED FORWARD, because the running tally above had
  // drifted: `*.integration.test.ts` files in this tree hold 183 cases across 20
  // files, all of them in `packages/adapters/postgres-tenancy`, of which
  // tranche 5's four suites are 39. The 159 this line used to claim was never
  // read off the tree.
...
test("the providers context is pinned at what vitest prints", () => {
  ...
  assert.equal(EXPECTED_RUNTIME_TOTAL, 717 + 375 + 149 + 306 + 378 + 605 + 352 + 254 + 288 + 515 + 362 + 269 + 609 + 198 + 25 + 29 + 59 + 34 + 1 + 350 + 56 + 67 + 43 + 41 + 33 + 60);
});
...
test("the skills adoption is pinned, and moved nothing else", () => {
  ...
  assert.equal(
    sum + 149 + 306 + 378 + 605 + 352 + 254 + 288 + 515 + 362 + 269 + 609 + 198 + 350 + 56 + 67 + 43 + 41 + 33 + 60,
    EXPECTED_RUNTIME_TOTAL,
  );
});
...
test("the conversations context is pinned at what vitest prints", () => {
  ...
  assert.equal(EXPECTED_RUNTIME_TOTAL, 5525 + 350 + 56 + 67 + 43 + 41 + 33 + 60);
});
...
test("the postgres-tenancy adapter is pinned at what vitest prints", () => {
  ...
  // 15/156 is the merged row, and either one taken alone would drop the other's
  ...
  assert.equal(
    ...
    17 + 14 + 16 + 9 + 17 + 3 + 16 + 11 + 7 + 4 + 9 + 7 + 12 + 8 + 10 + 6 + 11 + 12 + 7 + 3 + 14 + 7 + 16 + 12 + 9 + 2,
  );
  assert.equal(EXPECTED["packages/adapters/postgres-tenancy"].cases, 259);
  // 76 of the 259 run in `pnpm test:v1-packages`; the other 183 need a Docker
  // daemon and run in the `postgres-tenancy-repository` CI job. A pin that
  // counted only the runnable 76 would go green if the integration suites were
  // deleted, which is the one change this row exists to make visible.
  assert.equal(EXPECTED_RUNTIME_TOTAL, 5875 + 56 + 67 + 43 + 41 + 33 + 60);
  assert.equal(
    EXPECTED_RUNTIME_TOTAL,
    Object.values(EXPECTED).reduce((total, row) => total + row.cases, 0)
  );
