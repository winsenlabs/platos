...
test("the providers context is pinned at what vitest prints", () => {
  ...
  assert.equal(EXPECTED_RUNTIME_TOTAL, 717 + 375 + 149 + 306 + 378 + 605 + 352 + 254 + 288 + 515 + 362 + 269 + 609 + 198 + 25 + 29 + 59 + 34 + 1 + 350 + 56 + 67 + 43);
});
...
test("the skills adoption is pinned, and moved nothing else", () => {
  ...
  assert.equal(
    sum + 149 + 306 + 378 + 605 + 352 + 254 + 288 + 515 + 362 + 269 + 609 + 198 + 350 + 56 + 67 + 43,
    EXPECTED_RUNTIME_TOTAL,
  );
});
...
test("the conversations context is pinned at what vitest prints", () => {
  ...
  assert.equal(EXPECTED_RUNTIME_TOTAL, 5525 + 350 + 56 + 67 + 43);
});
...
test("the postgres-tenancy adapter is pinned at what vitest prints", () => {
  ...
  assert.equal(
    ...
    17 + 14 + 16 + 9 + 17 + 3 + 16 + 11 + 7 + 4 + 9 + 7 + 12 + 8 + 10 + 6,
  );
  assert.equal(EXPECTED["packages/adapters/postgres-tenancy"].cases, 166);
  // 55 of the 166 run in `pnpm test:v1-packages`; the other 111 need a Docker
  ...
  assert.equal(EXPECTED_RUNTIME_TOTAL, 5875 + 56 + 67 + 43);
  assert.equal(
    EXPECTED_RUNTIME_TOTAL,
    Object.values(EXPECTED).reduce((total, row) => total + row.cases, 0)
  );
