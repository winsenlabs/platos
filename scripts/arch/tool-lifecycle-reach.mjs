...
export const DEPENDENCY_SLOTS = Object.freeze({
  ...
  dispatch: {
    ...
    note: "the ToolDispatch port. ADR M0.3 §5.1 rule (h) binds `@modelcontextprotocol/*` to `packages/contexts/tools/(adapters|transport)/` and to nowhere else, so this port's implementation cannot be a `packages/adapters/` directory and cannot be a binding row; it is built in the composition root from the factory that home publishes, the way `Judge` is.",
  },
  digest: {
    kind: "root-satisfied",
    source: "packages/contexts/tools/adapters",
    note: "the ContentDigest port. `apps/core-api/src/composition/adapter-bindings.ts` states outright that ContentDigest is `a synchronous host hash with no failure channel and no row`, so it is satisfied where the dispatch is rather than by a binding.",
  },
