...
test("--check accepts the live generated tree and reports both ownership tiers", () => {
  ...
  // canonical-store ports it satisfies. Reasoned in full on EXPECTED_EDGE_COUNT
  // in the generator.
...
const LIVE_ADAPTERS = [
  { dir: "postgres-tenancy", port: "TenancyRepository", owner: "tenancy",
    additional: [
      ...
      // directory satisfies both, so the fixture carries four bindings on one
      // row. A copy that stopped at two would make every refusal below assert
...
test("§15 refusal: a SIXTEENTH binding fails, even though a directory may hold more than one", () => {
  ...
  assert.ok(errors.some((error) => error.includes("declares 15 adapter bindings; ADAPTERS flattens to 16")));
});
