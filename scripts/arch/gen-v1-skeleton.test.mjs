...
test("--check accepts the live generated tree and reports both ownership tiers", () => {
  ...
  // client. SEVEN edges for FIFTEEN bindings.
  // 106 -> 107 (WIN-258 T5): packages/adapters/postgres-tenancy ->
  // packages/contexts/observability, the THIRTEENTH owner of that same client.
  // ONE edge for ONE binding, over the ONE Prisma row ADR M0.3 §1 row 12 gives
  // that context. The count is READ BACK from the generator here rather than
...
test("§15 refusal: a FORTIETH binding fails, even though a directory may hold more than one", () => {
  // WIN-258 T5 moved this from thirty-one to forty across five tranches:
  // `providers`' one, `conversations`' four, `skills`' one, `memory`'s two and
  // `observability`'s one canonical-store bindings all landed in the one
  // directory.
