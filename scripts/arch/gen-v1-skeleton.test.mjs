...
test("§15 refusal: a FORTY-FIRST binding fails, even though a directory may hold more than one", () => {
  // WIN-258 T5 moved this from thirty-one to forty-one across five tranches:
  // `providers`' one, `conversations`' four, `skills`' one, `memory`'s two and
  // `jobs`' two canonical-store bindings all landed in the one directory.
  ...
  assert.ok(errors.some((error) => error.includes("declares 40 adapter bindings; ADAPTERS flattens to 41")));
});
