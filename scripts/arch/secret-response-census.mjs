...
/** Every code unit above ASCII, so a generated file's bytes cannot depend on an editor. */
const NON_ASCII = new RegExp("[^\\x00-\\x7F]", "gu");

/**
 * ASCII-only JSON with a trailing newline.
 *
 * WHY THE ESCAPES. This file is prose-heavy and the prose holds em dashes and
 * section signs. `JSON.stringify` emits them raw, so a regeneration on one
 * machine and the same regeneration on another can differ by encoding alone —
 * and a generated artifact whose bytes depend on the editor cannot be
 * byte-compared, which is the only way to prove a round trip. Escaping every
 * non-ASCII code unit makes the output a function of the tree and nothing else,
 * and it reproduces the committed file exactly.
 */
export function serializeManifest(manifest) {
  const escaped = JSON.stringify(manifest, null, 2).replace(NON_ASCII, (character) =>
...
/**
 * THE ROUND TRIP. `--write` now produces what `check()` READS.
 *
 * WHAT WAS BROKEN. `--write` printed a three-field object to stdout —
 * `{surfaces, totalOccurrences, sites:[{path,key,occurrences}]}` — and nothing
 * else. It dropped `indirectSites`, so feeding its output back to `check()`
 * threw `manifest.indirectSites is not iterable`; it dropped every
 * `disposition` and every `why`, which are the HALF OF THIS ARTIFACT A HUMAN
 * WROTE; and it wrote to a terminal rather than to the manifest. So the pin was
 * a hand edit, the "regenerator" could not regenerate, and the one instruction
 * in the file's own `$schema-note` — "generated by --write, then annotated" —
 * described a workflow that did not exist.
 *
 * THE RULE THAT MAKES IT SAFE. A regenerator that could invent a disposition
 * would be a way to launder a NEW raw-secret response path into a green
 * manifest, which is the exact failure this whole gate exists to prevent. So the
 * scanner supplies `path`/`key`/`occurrences`, the previous manifest supplies
 * `disposition`/`why` for every site that SURVIVES, and a site the previous
 * manifest has never seen is written with `null` for both — a shape `check()`
 * refuses with `BADDISP` and `NOWHY`. Regenerating a tree that grew a leak
 * therefore leaves the gate RED until somebody writes down why the leak is
 * there.
 *
 * `indirectSites` and the prose fields are carried through UNTOUCHED. They are
 * hand-found and weaker than the scanner (see `check()`), so there is nothing to
 * recompute; the only honest thing to do with them is not to lose them.
 *
 * @returns the manifest to write, plus the two sets that need human judgement:
 *   `introduced` (a site with no disposition) and `closed` (a pinned site that
 *   no longer occurs and has been dropped, so the note has to say what closed
 *   it).
 */
export function regenerate(root = repositoryRoot, previous = null) {
  const live = census(root);
  const held = previous ?? readManifest(root);
  const inherited = new Map(held.sites.map((entry) => [`${entry.path}#${entry.key}`, entry]));
  ...
  const sites = [...grouped].map(([id, occurrences]) => {
    const boundary = id.lastIndexOf("#");
    const carried = inherited.get(id);
    return {
      path: id.slice(0, boundary),
      key: id.slice(boundary + 1),
      occurrences,
      // `?? null` RATHER THAN OMITTING THE FIELD. An absent key and a null one
      // read the same way to `check()` — both fail `DISPOSITIONS.includes` and
      // both fail the `why` length test — but only the null one is VISIBLE in
      // the file a reviewer opens, which is where the missing reason has to be
      // noticed.
      disposition: carried?.disposition ?? null,
      why: carried?.why ?? null,
    };
  });

  return {
    // SPREAD FIRST so the prose fields, `indirectNote` and `indirectSites` keep
    // both their content and their POSITION: re-assigning a key that already
    // exists does not move it, so the regenerated file is a byte-for-byte match
    // for the committed one on an unchanged tree. That match is the round-trip
    // proof, and it is a case rather than a claim.
    manifest: {
      ...held,
    ...
    introduced: [...grouped.keys()].filter((id) => !inherited.has(id)).sort(),
    closed: [...inherited.keys()].filter((id) => !grouped.has(id)).sort(),
  };
}

function main() {
  if (process.argv.includes("--write")) {
    const { manifest, introduced, closed } = regenerate();
    writeFileSync(join(repositoryRoot, MANIFEST), serializeManifest(manifest));
    process.stdout.write(
      `secret-response-census --write: wrote ${MANIFEST} — ${manifest.sites.length} site(s), ` +
        `${manifest.totalOccurrences} occurrence(s) across ${manifest.surfaces} request surface(s), ` +
        `${manifest.indirectSites.length} indirect path(s) carried through unchanged\n`,
    );
    for (const id of introduced) {
      process.stdout.write(
        `NEW      ${id} was written with no disposition and no reason; the check stays RED ` +
          `until somebody says why this response returns secret material\n`,
      );
    }
    for (const id of closed) {
      process.stdout.write(
        `CLOSED   ${id} no longer occurs and its row has been REMOVED; say in the note what closed it\n`,
      );
    }
    // NON-ZERO ONLY WHEN A HUMAN IS OWED SOMETHING. An occurrence count that
    // moved or a surface count that grew are mechanical and exit 0; a new
    // undispositioned site or a dropped row are the two outcomes a regeneration
    // must not be able to slip past a reviewer.
    process.exitCode = introduced.length + closed.length > 0 ? 1 : 0;
    return;
  }
  const result = check();
