...
export function renderMarkdown(measurement) {
  ...
  lines.push("");
  // PLAIN PROSE, NOT AN HTML COMMENT. `docs/**` is built as MDX, and an
  // `<!-- … -->` line fails `pnpm audit:docs-build` with a parsing error before
  // any of this document's content is read. The sibling generated artifacts under
  // docs/audits/ say it in prose for the same reason.
  lines.push("Generated by `scripts/arch/mcp-tool-store-reach.mjs`; `--check` fails when it drifts from the tree.");
  lines.push("");
  lines.push(
