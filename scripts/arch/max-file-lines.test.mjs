...
test("the live selectors scan an exact nonzero source census", () => {
  ...
  // 74 -> 263 -> 328 -> 379. WIN-256 made packages/kernel and four contexts
  ...
  //
  // +51: THIS BRANCH adds `packages/contexts/jobs`. Its largest file is well
  // inside the 400-line warn threshold: the execution path is split across
  ...
  assert.equal(result.fileCount, 379);
  assert.deepEqual(result.errors, []);
