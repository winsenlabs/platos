...
test("the live selectors scan an exact nonzero source census", () => {
  ...
  //
  // THE TRANCHE-5 BLOCKS SUM: 1200 + 18 + 16 + 16 + 1 + 15 + 18 + 19 + 17 = 1320.
  ...
  // `secrets`, 1268 for `skills`.
  assert.equal(result.fileCount, 1320);
  // Written out so a DELETION CANNOT HIDE INSIDE AN ADDITION: adoption replaces
  // a context's four placeholders in place and adds the rest, so this number
  // only ever grows and a fall in it is always a finding.
  assert.equal(
    result.fileCount,
    328 + 44 + 55 + 51 + 77 + 63 + 48 + 48 + 67 + 56 + 42 + 83 + 8 + 4 + 20 + 54 + 18 + 74 + 12 + 22 + 11 + 9 + 6 + 18 + 16 + 16 + 1 + 15 + 18 + 19 + 17
  );
  ...
  // (secrets) + 17 (skills) = 234. The contexts, kernel and app rows are
  ...
  // widening one. `secrets` and `skills` are the sharpest cases: both had their
  // port entry point widened, in place, and a widened file is not a new one.
  assert.equal(result.fileCount, 20 + 1060 + 234 + 6);
  assert.deepEqual(result.errors, []);
