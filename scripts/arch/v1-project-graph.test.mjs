...
test("the live owner map passes its own check", () => {
  ...
  // `skills` the NINTH. None of `agents`, `governance` or `secrets` is more than
  // one owner edge even though they publish two ports, five and two, and neither
  // is `skills`, whose ONE port covers three tables: this map counts OWNERS, and
  // the project reference the adapter needs is per package, not per port.
