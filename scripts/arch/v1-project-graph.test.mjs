...
test("the live owner map passes its own check", () => {
  ...
  // `jobs` is a third: it publishes FOUR ports and gets ONE edge, because only
  // two of the four are canonical stores — `IdempotencyStore` is a reserve-once
  // keyspace and `JobHandlerRuntime` is an isolate, and neither writes a row.
