...
test("the live owner map passes its own check", () => {
  ...
  // 2 -> 9 (WIN-258 T5, seven times). `tools` is the THIRD owner delegated to
  ...
  // `providers` the NINTH. None of `agents`, `governance` or `secrets` is more
  // than one owner edge even though they publish two ports, five and two: this
  // map counts OWNERS, and the project reference the adapter needs is per
  // package, not per port. `providers` is the converse and the sharpest case:
  // it publishes THREE ports and gets ONE edge, because only one of the three
  // is a canonical store — `ModelRouter` belongs to `model-router-providers`
  // and `ProviderProbeCache` to no adapter at all.
  assert.deepEqual(EXPECTED_MULTI_OWNER_ADAPTERS, { "postgres-tenancy": 9 });
...
test("§15 refusal: the multi-owner adapter LOSING an edge it was granted fails too", () => {
  ...
  assert.ok(
    errors.some((error) =>
      error.includes("packages/adapters/postgres-tenancy expects 1 owner edge(s); 9 is what ADR M0.3 §4/§15 grants it")
    )
  );
});
