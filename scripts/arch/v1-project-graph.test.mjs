...
test("the live owner map passes its own check", () => {
  ...
  //
  // WIN-267 A2 moves them apart AGAIN, and in the same direction: 13 + 1 = 14
  // directories, and the multi-owner map is UNCHANGED, because
  // `tokenmint-totp` satisfies TWO of `identity-access`' ports through ONE
  // project reference. Two adjacent lines that move on different tranches is
  // exactly what asserting them separately is for.
