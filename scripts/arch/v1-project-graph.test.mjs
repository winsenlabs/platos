...
test("the live owner map passes its own check", () => {
  ...
  // 2 -> 4 (WIN-258 T5, twice). `tools` is the THIRD owner delegated to this one
  // directory and `agents` the FOURTH. `agents` is one owner edge and not two
  // even though it publishes two ports: this map counts OWNERS, and the project
  // reference the adapter needs is per package, not per port.
  assert.deepEqual(EXPECTED_MULTI_OWNER_ADAPTERS, { "postgres-tenancy": 4 });
...
test("§15 refusal: the multi-owner adapter LOSING an edge it was granted fails too", () => {
  ...
  assert.ok(
    errors.some((error) =>
      error.includes("packages/adapters/postgres-tenancy expects 1 owner edge(s); 4 is what ADR M0.3 §4/§15 grants it")
    )
  );
});
