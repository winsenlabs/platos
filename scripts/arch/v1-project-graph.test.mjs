...
test("the live owner map passes its own check", () => {
  ...
  // `memory` the NINTH. None of `agents`, `governance`, `secrets` or `memory` is
  // more than one owner edge even though they publish two ports, five, two and
  // two: this map counts OWNERS, and the project reference the adapter needs is
  // per package, not per port.
