...
test("the live owner map passes its own check", () => {
  ...
  // 2 -> 3 (WIN-258 T5). `agents` is the THIRD owner delegated to this one
  // directory, and it is one owner edge and not two even though it publishes two
  // ports: this map counts OWNERS, and the project reference the adapter needs is
