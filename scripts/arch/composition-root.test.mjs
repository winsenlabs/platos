...
test("the live repository satisfies both the boundary rules and the composition-root audit", () => {
  ...
  // 38 -> 39 (WIN-258 T5): `eventing` adds `NotificationRuleRepository`, its
  // ONE canonical-store port over the ONE canonical row of §1 row 17, so that
  // directory carries TWENTY-EIGHT and the thirteenth owner arrives without a
  // thirteenth directory. It is proven against the ADAPTER rather than through a
  // property, like `providers` above and unlike `secrets`' pair: its nine method
  // names collide with nothing the directory publishes, so no arbitration was
  // needed.
...
test("the binding-table parser reads all THIRTY-NINE bindings, across twelve directories", () => {
  ...
  // and WIN-258 T5 then added `eventing`'s ONE, over its ONE canonical row.
