...
test("the live repository satisfies both the boundary rules and the composition-root audit", () => {
  ...
  //
  // WIN-272 (M4.6) takes it to 60, and the DIRECTORY count is unmoved a THIRD
  // time: `redis-streams:StreamJournal` is a second row on the streams directory,
  // satisfied by the SAME object holding the SAME Redis client. A sixteenth
  ...
  // WIN-271 (M4.5), D10, THE REMAINING TWO PROVIDERS take it to 67 and the
  // DIRECTORY count to EIGHTEEN, the same case §15 does not consolidate twice
  // more: Meta and Telegram are two more vendors behind the same `channels` port
  // pair, so each is a new directory with two rows.
  assert.equal(audit.bindingCount, 67);
  //
  ...
  // WIN-271 (M4.5), D10: 15 -> 16, `channel-discord`; then 16 -> 18 with
  // `channel-whatsapp` and `channel-telegram`, the rest of D10's three.
  assert.equal(ADAPTERS.length, 18);
});
...
test("C2: an entry removed from the binding table fails", () => {
  ...
  assert.ok(problems.some((problem) => problem.includes("binding table omits channel-slack")));
    // 60 -> 61 (D20): the table declares 61 and this edit removes ONE of the two
    ...
    // 63 -> 67: D10's remaining four channel rows, so one row removed leaves 66.
    assert.ok(problems.some((problem) => problem.includes("declares 66 binding(s)")));
});
...
test("C7: an adapter that gains a real constructor must leave the list — the join reads the package", () => {
  // The evidence is the PACKAGE, not the list. Giving a still-unimplemented
  // directory a real export while leaving it declared unimplementable fails, and
  // it fails because of a file the composition root does not own.
  //
  // THE SUBJECT MOVED AT WIN-272 (M4.6), which is this control working. It used to
  // be `redis-streams`; that directory now HAS a constructor and has left the
  // list, so the edit would no longer be the counterfactual this case is about.
  // `durable-runtime` is the one M4.5 named as the remaining blocker on
  // `channels`, and it is still a generated interface.
  const root = realTreeCopy();
  edit(root, "packages/adapters/durable-runtime/src/index.ts", (source) =>
    `${source}\nexport { createDurableRuntimeAdapter } from "./adapter.js";\n`
  );
  assert.ok(
    auditCompositionRoot(root).problems.some((problem) =>
      problem.includes("UNIMPLEMENTED_ADAPTERS says durable-runtime cannot be constructed")
    )
  );
});
...
test("C7 NON-VACUITY: the live list names exactly the directories with no constructor", () => {
  ...
  // 6 -> 5 (WIN-272, M4.6). `redis-streams` is the THIRD directory ever to leave
  // this list, and the shape is A3's and M4.5's exactly: no directory is added,
  // the unimplemented count falls, and the constructible set below gains a TENTH
  // member without `ADAPTERS.length` moving.
  // 5 -> 4 (D20, 2026-09-15). `notifier-email` is the FOURTH directory ever to
  ...
  // 4 of 16 -> 4 of 18. Both new directories arrived WITH constructors, so
  // neither joins this set and neither moves its count while ADAPTERS.length
  // moves by two: 4 + 14 = 18.
  assert.equal(listed.length, 4, "four of the eighteen directories are still generated interfaces");
  ...
  // After D10's three channel directories the identity reads 4 + 14 = 18: the
  // unimplemented count has not moved for any of them, because each arrived with
  // a constructor and none was ever a generated interface.
  assert.deepEqual(constructible, [
    ...
    "channel-slack",
    // WIN-271 (M4.5), D10. The THIRTEENTH and FOURTEENTH, on the same terms as
    // the ELEVENTH: new directories that arrived with constructors rather than
    // generated interfaces filled in. This list is SORTED, so their position here
    // is alphabetical and not their declaration order.
    "channel-telegram",
    "channel-whatsapp",
    "keyring-envelope",
    ...
    "node-crypto-digest",
    // D20 (2026-09-15). The ELEVENTH constructible directory, and the fourth to
    // arrive by a generated interface gaining an implementation.
    "notifier-email",
    "outbox",
    "postgres-tenancy",
    "redis-cache",
    "redis-ratelimit",
    // WIN-272 (M4.6). The TENTH constructible directory, and the third to arrive
    // by an existing generated interface gaining an implementation rather than by
    // a new directory being written.
    "redis-streams",
    "tokenmint-totp",
  ]);
...
test("the binding-table parser reads all SIXTY-SEVEN bindings, across eighteen directories", () => {
  const source = readFileSync(join(repositoryRoot, COMPOSITION_ROOT_FILE), "utf8");
  ...
  // 63 -> 67: D10's remaining four channel rows.
  ...
  // WIN-271 (M4.5), D10: two rows each on the seventeenth and eighteenth.
  assert.equal(entries.filter((entry) => entry.adapter === "channel-whatsapp").length, 2);
  assert.equal(entries.filter((entry) => entry.adapter === "channel-telegram").length, 2);
  assert.equal(new Set(entries.map((entry) => entry.adapter)).size, 18);
});
...
test("§15 refusal: a binding table row the ADR does not declare fails", () => {
  ...
  assert.ok(
    auditCompositionRoot(root).problems.some((problem) =>
        problem.includes("binding table names outbox -> memory Cache, which is not one of the 67 declared bindings")
    )
  );
});
