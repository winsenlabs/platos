...
test("the live repository satisfies both the boundary rules and the composition-root audit", () => {
  ...
  // 30 -> 32 (WIN-258 T5): `memory` adds `MemoryRepository` and
  // `KnowledgeGraphRepository` over its three canonical rows, so that directory
  // carries TWENTY-ONE. Both are proven through the property that carries them,
  ...
  assert.equal(audit.bindingCount, 32);
...
test("C2: an entry removed from the binding table fails", () => {
  ...
  assert.ok(problems.some((problem) => problem.includes("declares 31 binding(s)")));
});
...
test("the binding-table parser reads all THIRTY-TWO bindings, across twelve directories", () => {
  const source = readFileSync(join(repositoryRoot, COMPOSITION_ROOT_FILE), "utf8");
  ...
  // A directory with nineteen bindings appears NINETEEN TIMES in the
  ...
  // them, and WIN-258 T5 then added `memory`'s two.
  // 1 + 1 + 1 + 2 + 1 + 1 + 5 + 2 + 5 + 2 = 21.
  assert.equal(entries.filter((entry) => entry.adapter === "postgres-tenancy").length, 21);
...
test("§15 refusal: a binding table row the ADR does not declare fails", () => {
  ...
  assert.ok(
    auditCompositionRoot(root).problems.some((problem) =>
      problem.includes("binding table names outbox -> memory Cache, which is not one of the 32 declared bindings")
    )
  );
});
