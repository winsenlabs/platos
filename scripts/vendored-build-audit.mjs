...
// WHAT "PROTECTED" MEANS AFTER WIN-270, AND WHY IT HAD TO CHANGE.
//
// WIN-253 pinned these seven trees BYTE-IDENTICAL to the integration base, and
// that was the right rule for what it was proving: a removal audit's most
// dangerous failure is collateral damage, so "we deleted six inherited
// workspaces and did not touch the seven Platos ones" had to be a fact rather
// than an assurance. Nothing about that reading was wrong.
//
// It was, however, written as a STANDING invariant against a frozen commit, and
// the trees it froze are the published SDKs. So it forbade not only collateral
// damage but every deliberate change any later milestone could make to them —
// including WIN-270 (M4.4), whose entire subject is aligning those SDKs to V1.
// A gate that makes its own milestone's work impossible has stopped measuring
// collateral damage and started measuring elapsed time.
//
// SO THE RULE IS NARROWED IN THE ONE WAY THAT KEEPS IT A GATE. Deviation from
// the base is still a violation BY DEFAULT; what changes is that a deviation can
// be REVIEWED, by exactly the mechanism this file already uses twice above for
// `allowedPrimaryBaseAdditions` and `allowedAdditionalIntegrationDeletions` — an
// explicit path, spelled out, with a reason a human wrote. Three properties are
// deliberately kept:
//
//   A FILE MAY STILL NEVER VANISH. `protected SDK file is missing` has no
//   reviewed form at all. Deletion is the collateral damage WIN-253 was actually
//   guarding against, and no milestone needs to delete a published SDK file to
//   change it.
//
//   AN UNLISTED CHANGE IS STILL A VIOLATION. Adding a file or editing one
//   without a line here fails, so the gate still catches the change nobody
//   meant to make.
//
//   A LISTED PATH THAT IS NOT ACTUALLY DEVIATING IS ALSO A VIOLATION. Without
//   that, this table would accumulate permissions for changes that were later
//   reverted, and the next tranche would inherit a hole shaped like whatever the
//   last one happened to touch.
//
// `byteIdentical` survives on the receipt and still means byte-identical; the
// four trees WIN-270 did not touch still report `true`.
const allowedProtectedSdkChanges = [
  {
    path: "packages/platos-client/src/errors.ts",
    reason:
  ...
  {
    path: "packages/platos-client/src/generated/v1.ts",
    reason:
      "WIN-270 (M4.4): GENERATED by `pnpm generate:sdk-v1` from the V1 OpenAPI document, the operation manifest and core-api's idempotency policy. `pnpm audit:sdk-v1` fails when it differs from what those inputs emit.",
  },
  {
    path: "packages/platos-client/src/v1-transport.ts",
    reason:
      "WIN-270 (M4.4): the hand-written half of the V1 client — auth, retry, and the `Idempotency-Key` minted once per logical call so a retried mint replays instead of minting a second credential.",
  },
  {
    path: "packages/platos-client/tests/v1-contract.test.ts",
    reason:
      "WIN-270 (M4.4): drives the generated client against `tests/sdk-contract/v1-fixtures.json`, the same fixture the Python suite drives.",
  },
  {
    path: "packages/platos-client-py/.gitignore",
    reason:
  ...
  {
    path: "packages/platos-client-py/platos_client/generated/__init__.py",
    reason: "WIN-270 (M4.4): the package marker for the generated V1 module beside it.",
  },
  {
    path: "packages/platos-client-py/platos_client/generated/v1.py",
    reason:
      "WIN-270 (M4.4): GENERATED by `pnpm generate:sdk-v1` from the same three inputs as its TypeScript sibling, in one pass, so the two languages cannot disagree.",
  },
  {
    path: "packages/platos-client-py/platos_client/v1_transport.py",
    reason:
      "WIN-270 (M4.4): the hand-written Python transport. Standard library only and synchronous, so `pnpm test:sdk-v1` can execute the cross-language fixture on a runner with an interpreter and no packages.",
  },
  {
    path: "packages/platos-client-py/tests/test_v1_contract.py",
    reason:
      "WIN-270 (M4.4): the Python half of the cross-language fixture. Imports nothing third-party and carries its own runner.",
  },
  {
    path: "packages/platos-embed/src/embed.ts",
    reason:
      "WIN-270 (M4.4): `this.iframe` went on pointing at a detached element after a missing-attribute refusal, and that reference is what `onMessage` compares an event's `source` against. The handle is now dropped with the frame.",
  },
  {
    path: "packages/platos-embed/tests/embed.test.ts",
    reason:
      "WIN-270 (M4.4): holds the missing-attribute refusal to being total — no iframe in the tree and no live handle behind it — and records that this package makes no authenticated request of its own.",
  },
  {
    path: "packages/platos-react-widget/src/usePlatosChat.ts",
    reason:
      "WIN-270 (M4.4): the widget's only unauthenticated call now throws a coded refusal, and the optimistic assistant bubble is withdrawn when the turn never reached the agent, so a declined visitor does not get a transcript reading `[error]`.",
  },
  {
    path: "packages/platos-react-widget/tests/usePlatosChat.test.tsx",
    reason:
      "WIN-270 (M4.4): the refusal cases for the above, and a mock factory that keeps the real error helpers instead of stubbing the thing under test.",
  },
  {
    path: "packages/platos-token-mint/src/index.ts",
    reason:
...
function validateProtectedTrees(root, currentPaths, violations, options = {}) {
  const reviewed = new Map(
    (options.allowedProtectedSdkChanges ?? allowedProtectedSdkChanges).map((entry) => [entry.path, entry]),
  );
  ...
  const trees = protectedRoots.map((path) => {
    const entries = allEntries.filter(({ path: entryPath }) => entryPath.startsWith(`${path}/`));
    const deviations = [];
    for (const entry of entries) {
      const absolute = resolve(root, entry.path);
      if (!existsSync(absolute)) {
        // NO REVIEWED FORM. A published SDK file may be edited, never deleted;
        // deletion is the collateral damage this gate was built to catch.
        violations.push(`protected SDK file is missing: ${entry.path}`);
        continue;
      }
      if (sha256(readFileSync(absolute)) === sha256(allBlobs.get(entry.path))) continue;
      const review = reviewed.get(entry.path);
      if (review === undefined) {
        violations.push(`protected SDK file changed from integration base: ${entry.path}`);
        continue;
      }
      consumed.add(entry.path);
      deviations.push({ path: entry.path, kind: "changed", reason: review.reason });
    }
    ...
    for (const currentPath of currentPaths.filter((entryPath) => entryPath === path || entryPath.startsWith(`${path}/`))) {
      if (basePaths.has(currentPath)) continue;
      const review = reviewed.get(currentPath);
      if (review === undefined) {
        violations.push(`protected SDK tree gained a path: ${currentPath}`);
        continue;
      }
      consumed.add(currentPath);
      deviations.push({ path: currentPath, kind: "added", reason: review.reason });
    }
    deviations.sort((left, right) => (left.path < right.path ? -1 : 1));
    return {
      path,
      ...
      byteIdentical: deviations.length === 0 && !violations.some((violation) => violation.includes(path)),
      reviewedChanges: deviations,
    };
  });
  // A REVIEWED PATH THAT IS NOT DEVIATING IS A HOLE. Without this the table
  // would accumulate permissions for changes somebody later reverted, and the
  // next tranche would inherit an exemption shaped like whatever the last one
  // touched.
  for (const [path] of reviewed) {
    if (consumed.has(path)) continue;
    violations.push(`reviewed protected SDK change no longer deviates from the integration base: ${path}`);
  }
  return trees;
}
...
export function auditRepository(root = repositoryRoot, options = {}) {
  ...
  const protectedTrees = validateProtectedTrees(root, currentPaths, violations, {
    allowedProtectedSdkChanges: options.allowedProtectedSdkChanges,
  });
...
export function markdownText(report) {
  const protectedRows = report.protectedTrees
    .map(({ path, integrationBaseTreeOid, trackedFiles, byteIdentical, reviewedChanges }) => `| \`${path}\` | \`${integrationBaseTreeOid}\` | ${trackedFiles} | ${byteIdentical ? "yes" : "no"} | ${(reviewedChanges ?? []).length} |`)
    .join("\n");
  const reviewedChangeRows = report.protectedTrees
    .flatMap(({ reviewedChanges }) => reviewedChanges ?? [])
