...
to them. The doc comments now say so. Documentation only; no export, type or wire
field changed.

The Python `platools` package carries the same correction on `current_scope()`
and its module example. It has no npm identity, so its version intent is recorded
