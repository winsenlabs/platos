...
function fullySupplied(): SuppliedAdapters {
  const names = [
    ...
    "keyring-envelope",
    // WIN-267 A2. The fourteenth, listed by name for the same reason and with an
    // extra one: this is the only directory an install gets WITHOUT configuring
    // anything, so deriving the list from `ADAPTER_NAMES` would have hidden the
    // one case where readiness could go green on a slot nobody filled.
    "tokenmint-totp",
  ];
