...
describe("the declared binding table", () => {
  it("declares TWENTY-TWO bindings across ADR M0.3 §4's TWELVE adapter directories", () => {
    ...
    // `BudgetRepository` and `governance`'s five, because there is one
    // PostgreSQL database behind one client. All eleven are asserted, and the
    // gap between the two counts is asserted too, so a change that collapsed
    // them back into one count fails.
    //
    // 12 directories + 15 extra ports on the one shared directory = 27 bindings.
    ...
    expect(ADAPTER_BINDINGS).toHaveLength(27);
    expect(DECLARED_BINDING_COUNT).toBe(27);
  ...
  it("names each adapter DIRECTORY exactly once, even though one has ELEVEN bindings", () => {
...
describe("adapter supply validation", () => {
  it("reports every binding unsatisfied when nothing is wired — the honest M2.1b state", () => {
    ...
    expect(report.unsatisfied).toHaveLength(27);
    expect(report.faults).toEqual([]);
    expect(describeAdapterSupply(report)).toBe("0/27 adapter bindings satisfied");
    // Reported per BINDING, not per directory. A directory-named report would
    // list `postgres-tenancy` once and say 12/12 while SIXTEEN of the ports it
  ...
  it("accepts an adapter that identifies its own slot", () => {
    ...
    expect(report.unsatisfied).toHaveLength(26);
...
describe("composing the application", () => {
  it("composes with nothing wired and reports the gap rather than pretending", () => {
    ...
    expect(app.bindings.unsatisfied).toHaveLength(27);
  ...
  it("records a satisfied binding and leaves the rest unsatisfied", () => {
    ...
    expect(app.bindings.unsatisfied).toHaveLength(26);
