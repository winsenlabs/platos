...
describe("the declared binding table", () => {
  it("declares FORTY bindings across ADR M0.3 §4's TWELVE adapter directories", () => {
    ...
    //
    // 12 directories + 28 extra ports on the one shared directory = 40 bindings.
    ...
    expect(ADAPTER_BINDINGS).toHaveLength(40);
    expect(DECLARED_BINDING_COUNT).toBe(40);
    ...
    // The thirteenth is `jobs`, whose TWO ports over `Job` and `AgentApproval`
  ...
  it("names each adapter DIRECTORY exactly once, even though one has TWENTY-NINE bindings", () => {
...
describe("adapter supply validation", () => {
  it("reports every binding unsatisfied when nothing is wired — the honest M2.1b state", () => {
    ...
    expect(report.unsatisfied).toHaveLength(40);
    expect(report.faults).toEqual([]);
    expect(describeAdapterSupply(report)).toBe("0/40 adapter bindings satisfied");
  ...
  it("accepts an adapter that identifies its own slot", () => {
    ...
    expect(report.unsatisfied).toHaveLength(37);
...
describe("composing the application", () => {
  it("composes with nothing wired and reports the gap rather than pretending", () => {
    ...
    expect(app.bindings.unsatisfied).toHaveLength(40);
  ...
  it("records a satisfied binding and leaves the rest unsatisfied", () => {
    ...
    expect(app.bindings.unsatisfied).toHaveLength(37);
