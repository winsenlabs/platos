...
describe("the declared binding table", () => {
  it("declares FOURTEEN bindings across ADR M0.3 §4's TWELVE adapter directories", () => {
    ...
    // `IdentityAccessRepository` and — since tranche 5 — `BudgetRepository`,
    // because there is one PostgreSQL database behind one client. Both are
    // asserted, and the gap between them is asserted too, so a change that
    ...
    const multiPort = ADAPTER_BINDINGS.filter((binding) => binding.adapter === "postgres-tenancy");
    expect(multiPort.map((binding) => binding.port)).toEqual([
      "TenancyRepository",
      "IdentityAccessRepository",
      "BudgetRepository",
    ]);
    expect(multiPort.map((binding) => binding.owner)).toEqual([
      "tenancy",
      "identity-access",
      "cost-monitoring",
    ]);
...
describe("adapter supply validation", () => {
  it("reports every binding unsatisfied when nothing is wired — the honest M2.1b state", () => {
    ...
    // list `postgres-tenancy` once and say 12/12 while three ports went
    // unserved, which is a readiness endpoint that lies about what is serving.
