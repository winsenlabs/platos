...
describe("the declared binding table", () => {
  it("declares THIRTY-NINE bindings across ADR M0.3 §4's TWELVE adapter directories", () => {
    ...
    // The THIRTEENTH is `eventing`, whose ONE port covers its ONE canonical
    // row — the smallest grant in the map, and proven against the ADAPTER
    // rather than through a property, because its nine method names collide
    // with nothing the directory already publishes.
