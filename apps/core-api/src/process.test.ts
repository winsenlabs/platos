...
describe("the built binary starts, serves and stops", () => {
  ...
  it("reports a REAL binding count on /readyz once the stores are declared", async () => {
    ...
    expect(body.detail.declaredBindings).toBe(57);
    expect(body.detail.satisfiedBindings).toHaveLength(50);
    ...
    expect(body.reason).toBe("50 of 57 adapter bindings are satisfied; 7 are not");
    ...
    expect(spawned.stdout()).toContain("50/57 adapter bindings satisfied");
