 ...
 *
 * TWELVE SLOTS, THIRTY-FOUR BINDINGS (ADR M0.3 §15). An install wires a
 ...
 * and adding a row for it would claim a thirty-first binding the ADR does not
...
export const ADAPTER_BINDINGS: readonly AdapterBinding[] = Object.freeze([
  ...
  // WIN-258 T5 (ADR M0.3 §15). The FIFTEENTH through EIGHTEENTH bindings of the
  // same directory, and the NINTH owner of the one PostgreSQL client. They are
  ...
  // WIN-258 M2.3 — TENANCY'S FIVE NON-REPOSITORY PORTS, the NINETEENTH through
  // TWENTY-THIRD bindings of the same directory.
  //
  // They are a different KIND of binding from the eighteen above and that is why
 ...
 *
 * De-duplicated because `ADAPTER_BINDINGS` now holds thirty-four rows across
 * twelve directories: a caller iterating this list to construct or close
 * adapters would otherwise build `postgres-tenancy` TWENTY-THREE times and open
 * twenty-three pools over the one database.
 */
