 ...
 *
 * TWELVE SLOTS, FORTY BINDINGS (ADR M0.3 §15). An install wires a
...
export const ADAPTER_BINDINGS: readonly AdapterBinding[] = Object.freeze([
  ...
  // the THIRTEENTH owner of the one PostgreSQL client. Appended after `memory`'s
  // pair for the reason that pair was appended after tenancy's five: every block
 ...
 *
 * De-duplicated because `ADAPTER_BINDINGS` now holds FORTY rows across twelve
 * directories: a caller iterating this list to construct or close adapters would
 * otherwise build `postgres-tenancy` TWENTY-NINE times and open twenty-nine
