...
describe("WIN-267 R1 — the finding that no V1 REST route could spend an authentication budget, REVISITED", () => {
  ...
  it("INVITE_ACCEPT: `acceptInvitation` IS published, on tenancy, and its route spends the budget before tenancy sees a token", async () => {
    ...
    expect(refusal, "INVITE_ACCEPT must refuse a guesser within twenty-one attempts").toBe("RATE_LIMITED");
    expect(admitted).toBeGreaterThan(0);
    // SPENT FIRST: every attempt tenancy saw was an admitted one, and the refused
    // attempt never reached the token lookup.
    ...
    expect(await attempt()).not.toBe("RATE_LIMITED");
