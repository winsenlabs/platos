...
/**
 * Kept for compatibility with call sites that passed `isCompleted`/`logCount` — under V2 we only care
 * about the live presence stream.
 */
export function useCrossEngineIsConnected(_args?: { isCompleted: boolean; logCount: number }) {
        ...
        <div className="flex flex-col items-center justify-center gap-6 px-6 py-10">
          ...
          <Paragraph variant="small" className={isConnected ? "text-success" : "text-error"}>
            {isConnected === undefined
              ? "Checking connection..."
              : isConnected
              ? "Your local dev server is connected to Platos"
              : "Your local dev server is not connected to Platos"}
          </Paragraph>
        </div>
        {isConnected ? null : (
          <div className="space-y-3">
            ...
            <Paragraph variant="small">
              ...
              your local workspace to Platos. The CLI is the upstream{" "}
              <InlineCode variant="extra-small">trigger.dev</InlineCode> durable-task CLI that
              Platos is built on. Keep it running while you develop to stay connected. Learn more
              in the <TextLink to={docsPath("cli-dev")}>CLI docs</TextLink>.
            </Paragraph>
          </div>
        )}
      </div>
