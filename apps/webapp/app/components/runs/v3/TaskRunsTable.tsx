...
import { formatDuration, formatDurationMilliseconds } from "@platos/core/v3";
...
import { type PossibleAgent } from "./RunFilters";
...
type RunsTableProps = {
  ...
  /**
   * Theme K.2 — Platos agents scoped to the current env. When present, the
   * table shows an "Agent" column (badge with the agent name) for any run
   * whose metadata contains an `agentId`. The column is hidden entirely if
   * none of the currently-visible runs have an agentId.
   */
...
export function TaskRunsTable({
  ...
  // Theme K.2 — derive per-run agentId from metadata + collapse the column
  // when zero visible runs carry one. Metadata is a JSON String? column so
  // we parse defensively and ignore non-objects / bad JSON.
  ...
  for (const run of runs) {
    ...
    try {
      ...
      if (parsed && typeof parsed === "object") {
        const candidate = (parsed as Record<string, unknown>).agentId;
        if (typeof candidate === "string" && candidate.length > 0) {
          agentIdByRun.set(run.id, candidate);
        }
      }
    } catch {
      // Bad metadata JSON — skip.
    }
  }
  const showAgentColumn = agentIdByRun.size > 0;
  const agentNameById = new Map<string, string>(
    (possibleAgents ?? []).map((a) => [a.id, a.name])
  );
    ...
    <Table variant={variant} className="max-h-full overflow-y-auto">
      <TableHeader>
        <TableRow>
          ...
          {showAgentColumn && <TableHeaderCell>Agent</TableHeaderCell>}
      ...
      <TableBody>
        {total === 0 && !hasFilters ? (
          <TableBlankRow colSpan={showAgentColumn ? 16 : 15}>
            ...
            return (
              <TableRow key={run.id}>
                ...
                {showAgentColumn && (
                  <TableCell to={path}>
                    {(() => {
                      const runAgentId = agentIdByRun.get(run.id);
                      if (!runAgentId) return <span className="text-text-dimmed">–</span>;
                      const name = agentNameById.get(runAgentId);
                      return (
                        <Badge variant="extra-small">
                          <span className="truncate">{name ?? runAgentId}</span>
                        </Badge>
                      );
                    })()}
                  </TableCell>
...
function BlankState({ isLoading, filters }: Pick<RunsTableProps, "isLoading" | "filters">) {
  ...
  if (
    ...
    Object.values(otherFilters).every((filterArray) => !Array.isArray(filterArray) || filterArray.length === 0)
  ) {
    return (
