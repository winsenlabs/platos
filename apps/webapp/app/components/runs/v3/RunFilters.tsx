...
import { MachinePresetName } from "@platos/core/v3";
import type { BulkActionType, TaskRunStatus, TaskTriggerSource } from "@platos/database";
...
export const TaskRunListSearchFilters = z.object({
  ...
  agentId: z
    ...
    .optional()
    .describe(
      "Platos agent ID to filter runs by. Matches runs whose metadata.agentId or 'agent:<id>' tag matches."
    ),
  threadId: z
    .string()
    .optional()
    .describe(
      "Platos agent thread ID to filter runs by. Matches runs whose metadata.threadId equals this value."
    ),
});
...
export type PossibleAgent = { id: string; name: string; slug: string };
export type PossibleThread = { id: string; title: string | null };
...
type RunFiltersProps = {
  ...
  /** Platos agents scoped to the current (org, project, env). Used to populate the Agent filter dropdown. */
  possibleAgents?: PossibleAgent[];
  /** Recent Platos agent threads scoped to the current (org, project, env). Used to populate the Thread filter dropdown. */
  possibleThreads?: PossibleThread[];
...
export function RunsFilters(props: RunFiltersProps) {
  ...
  const hasFilters =
    ...
    searchParams.has("agentId") ||
    searchParams.has("threadId");
...
const filterTypes = [
  ...
  { name: "agent", title: "Agent", icon: <CpuChipIcon className="size-4" /> },
  { name: "thread", title: "Thread", icon: <ChatBubbleLeftRightIcon className="size-4" /> },
...
function AppliedFilters({
  ...
  return (
    <>
      ...
      <AppliedAgentFilter possibleAgents={possibleAgents ?? []} />
      <AppliedThreadFilter possibleThreads={possibleThreads ?? []} />
      <AppliedTagsFilter />
...
function Menu(props: MenuProps) {
  switch (props.filterType) {
    ...
    case "agent":
      return <AgentDropdown onClose={() => props.setFilterType(undefined)} {...props} />;
    case "thread":
      return <ThreadDropdown onClose={() => props.setFilterType(undefined)} {...props} />;
  }
}
...
// Theme K.2 — Agent-centric filters on the Runs list. Single-select dropdowns
// populated from the presenter's scoped `possibleAgents` / `possibleThreads`.
// Each dropdown writes `agentId` / `threadId` as a single string query param and
...
function AgentDropdown({
  ...
  const filtered = useMemo(() => {
    ...
    return (possibleAgents ?? []).filter(
      (a) => a.name.toLowerCase().includes(q) || a.slug.toLowerCase().includes(q)
    );
  }, [searchValue, possibleAgents]);

  return (
    <SelectProvider value={value("agentId") ?? ""} setValue={handleChange} virtualFocus={true}>
      {trigger}
      <SelectPopover
        className="min-w-0 max-w-[min(360px,var(--popover-available-width))]"
        hideOnEscape={() => {
      ...
      >
        <ComboBox placeholder={"Filter by agent..."} value={searchValue} />
        <SelectList>
          <SelectItem value={""}>None</SelectItem>
          {filtered.length === 0 ? (
            <SelectItem disabled>No agents found</SelectItem>
          ) : (
            filtered.map((agent, index) => (
              ...
              >
                <span className="flex items-center gap-2">
                  ...
                  <span className="truncate text-text-dimmed">({agent.slug})</span>
                </span>
              </SelectItem>
            ))
          )}
...
function AppliedAgentFilter({ possibleAgents }: { possibleAgents: PossibleAgent[] }) {
  const { value, del } = useSearchParams();
  const agentId = value("agentId");
  if (!agentId) {
    return null;
  }
  const match = possibleAgents.find((a) => a.id === agentId);
  const display = match ? `${match.name} (${match.slug})` : agentId;
        ...
        <AgentDropdown
          trigger={
            <Ariakit.Select render={<div className="group cursor-pointer focus-custom" />}>
              <AppliedFilter
                label="Agent"
                icon={filterIcon("agentId")}
                value={display}
                onRemove={() => del(["agentId", "cursor", "direction"])}
                variant="secondary/small"
              />
            </Ariakit.Select>
          }
...
function threadDisplay(thread: PossibleThread) {
  const title = thread.title && thread.title.length > 0 ? thread.title : "Untitled thread";
  const truncated = title.length > 40 ? `${title.slice(0, 40)}…` : title;
  const idPrefix = thread.id.slice(0, 8);
  return `${truncated} | ${idPrefix}`;
}
...
function ThreadDropdown({
  ...
  const filtered = useMemo(() => {
    const q = searchValue.toLowerCase();
    return (possibleThreads ?? []).filter((t) => {
      const title = (t.title ?? "").toLowerCase();
      return title.includes(q) || t.id.toLowerCase().includes(q);
    });
  }, [searchValue, possibleThreads]);

  return (
    <SelectProvider value={value("threadId") ?? ""} setValue={handleChange} virtualFocus={true}>
      {trigger}
      <SelectPopover
        className="min-w-0 max-w-[min(420px,var(--popover-available-width))]"
        hideOnEscape={() => {
          if (onClose) {
            onClose();
            return false;
      ...
      >
        <ComboBox placeholder={"Filter by thread..."} value={searchValue} />
        <SelectList>
          <SelectItem value={""}>None</SelectItem>
          {filtered.length === 0 ? (
            <SelectItem disabled>No threads found</SelectItem>
          ) : (
            filtered.map((thread) => (
              <SelectItem key={thread.id} value={thread.id}>
                <MiddleTruncate text={threadDisplay(thread)} />
              </SelectItem>
            ))
          )}
        </SelectList>
...
function AppliedThreadFilter({ possibleThreads }: { possibleThreads: PossibleThread[] }) {
  const { value, del } = useSearchParams();
  const threadId = value("threadId");
  if (!threadId) {
    return null;
  }
  const match = possibleThreads.find((t) => t.id === threadId);
  const display = match ? threadDisplay(match) : threadId;
        ...
        <ThreadDropdown
          trigger={
            <Ariakit.Select render={<div className="group cursor-pointer focus-custom" />}>
              <AppliedFilter
                label="Thread"
                icon={filterIcon("threadId")}
                value={display}
                onRemove={() => del(["threadId", "cursor", "direction"])}
                variant="secondary/small"
              />
            </Ariakit.Select>
          }
