...
import { ErrorId, RunId } from "@platos/core/v3/isomorphic";
import {
...
export class ClickHouseRunsRepository implements IRunsRepository {
  ...
  async listRuns(options: ListRunsOptions) {
    ...
    // Theme K.2 — metadata-substring narrowing for Platos agent / thread
    // filters. TaskRun.metadata is a `String?` column storing the JSON blob
    // produced by `JSON.stringify`, so `contains` maps to a Postgres ILIKE
    // against a stable, whitespace-free needle (`"<key>":"<id>"`). We can't
    // use Prisma's `{ path, equals }` JSON operator here because that only
    // works against `Json`-typed columns — see docstring on the column.
    const metadataContains: string[] = [];
    if (options.agentId) {
      metadataContains.push(`"agentId":"${options.agentId}"`);
    }
    if (options.threadId) {
      metadataContains.push(`"threadId":"${options.threadId}"`);
    }
    ...
    let runs = await this.options.prisma.taskRun.findMany({
      where: {
        ...
        ...(metadataContains.length > 0
          ? { AND: metadataContains.map((needle) => ({ metadata: { contains: needle } })) }
          : {}),
      },
      orderBy: {
        id: "desc",
...
function applyRunFiltersToQueryBuilder<T>(
  ...
  // Theme K.2 — agent-centric narrowing at ClickHouse layer.
  //
  // `trigger-integration.ts` tags every agent-spawned run with
  //   tags: [org:<id>, project:<id>, env:<id>, agent:<agentId>]
  // so an exact `agent:<id>` match via `hasAny(tags, ...)` is cheap + indexed.
  //
  // threadId is only recorded in TaskRun.metadata today (not in tags), so we
  // defer that narrowing to the Prisma post-fetch step in `listRuns`.
  if (options.agentId) {
    queryBuilder.where("hasAny(tags, {agentTags: Array(String)})", {
      agentTags: [`agent:${options.agentId}`],
    });
  }
}
