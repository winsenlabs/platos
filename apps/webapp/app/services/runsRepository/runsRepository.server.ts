...
import { type Logger, type LogLevel } from "@platos/core/logger";
import { MachinePresetName } from "@platos/core/v3";
import { BulkActionId, RunId } from "@platos/core/v3/isomorphic";
import { type Prisma, TaskRunStatus } from "@platos/database";
...
const RunListInputOptionsSchema = z.object({
  ...
  // Theme K.2 — Platos agent-centric filters. Both match against
  // TaskRun.metadata (String? JSON blob) via a substring contains match,
  // and — for agentId — additionally narrow the ClickHouse query via the
  // denormalised `agent:<id>` tag that `trigger-integration.ts` already
  // sets on every agent-spawned run.
