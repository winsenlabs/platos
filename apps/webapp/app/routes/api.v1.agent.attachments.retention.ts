 ...
 * Gated by `X-Platos-Internal-Auth` — same pattern as the cost-catalog ingest
...
export async function action({ request }: ActionFunctionArgs) {
  ...
  // PLATOS_INTERNAL_AUTH_TOKEN was unset — the scheduled trigger.dev task saw
  ...
  if (!expected) {
    logger.error(
      "PLATOS_INTERNAL_AUTH_TOKEN not configured — attachment retention sweep refusing to run. Set the env var on the webapp container.",
    );
    return json(
      {
        status: "error",
        ...
        message:
          "PLATOS_INTERNAL_AUTH_TOKEN not configured on webapp. Retention sweep disabled until set.",
      },
      { status: 412 }
    );
  }
  ...
  const provided = request.headers.get("x-platos-internal-auth");
  const isValid =
