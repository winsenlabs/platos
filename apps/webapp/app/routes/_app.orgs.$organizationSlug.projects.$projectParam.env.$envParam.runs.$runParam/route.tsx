...
import { BATCH_TASK_IDENTIFIER, BatchRunView } from "./BatchRunView";
...
/**
 * K.1 — Pull agent+thread identifiers off a run's JSON-serialised metadata
 * blob. The Platos agent runtime writes `agentId` + `threadId` as metadata
 * keys on every task it spawns (see `agent-tool-block.task.ts` and
 * `agent-batch.task.ts`). Runs with no metadata, a non-object root, or a
 * missing agentId are not agent runs — return null.
 */
function parseAgentMetadata(raw: string | null): { agentId: string; threadId: string | null } | null {
  if (!raw) return null;
  try {
...
/**
 * K.1 — Fetch the agent + thread rows pointed at by run metadata. Both
 * queries are scope-gated with the full `(organizationId, projectId,
 * environmentId)` tuple. Either row may come back null (deleted, or the
 * metadata references a row outside the current scope) — the card renders a
 * fail-graceful message in that case instead of throwing.
 */
async function loadAgentContextForRun(params: {
  metadata: string | null;
  scope: { organizationId: string; projectId: string; environmentId: string } | null;
}) {
  if (!params.scope) return null;
  const parsed = parseAgentMetadata(params.metadata);
  if (!parsed) return null;

  const [agent, thread] = await Promise.all([
    $replica.platosAgent.findFirst({
      where: {
        id: parsed.agentId,
    ...
    parsed.threadId
      ? $replica.platosAgentThread.findFirst({
          where: {
            id: parsed.threadId,
            organizationId: params.scope.organizationId,
            projectId: params.scope.projectId,
          ...
          select: { id: true, title: true, turnCount: true, sessionContext: true },
        })
  ...
  return {
    agent,
    thread,
    missingAgentId: parsed.agentId,
    missingThreadId: parsed.threadId,
  };
}
...
export const loader = async ({ request, params }: LoaderFunctionArgs) => {
  ...
  // K.1 — Scope-gate the agent+thread lookup with the full tuple. We need
  // projectId for the findFirst guards; the presenter only exposes
  // organizationId + environmentId, so resolve the project here.
  const projectForScope = await findProjectBySlug(organizationSlug, projectParam, userId);
  const agentContext = projectForScope
    ? await loadAgentContextForRun({
        metadata: result.run.metadata ?? null,
        scope: {
          organizationId: result.run.environment.organizationId,
          projectId: projectForScope.id,
          environmentId: result.run.environment.id,
        },
      })
    : null;
...
export default function Page() {
  const { run, trace, maximumLiveReloadingSetting, runsList, resizable, agentContext } =
  ...
  return (
    <>
      ...
      {/*
        K.1 — Agent context banner: only rendered when the run was spawned by
        the Platos agent runtime (metadata.agentId). Placed outside PageBody
        so it doesn't fight the trace view's fixed-height resizable panels,
        and sits alongside the BatchRunView for `platos-agent-batch` runs.
      */}
      {agentContext ? (
        <div className="border-b border-charcoal-800 bg-background-dimmed px-3 py-3">
          <AgentContextCard
            agent={agentContext.agent}
            thread={agentContext.thread}
            missingAgentId={agentContext.missingAgentId}
            missingThreadId={agentContext.missingThreadId}
      ...
      {/*
        W.3 — Batch viewer short-circuits the trace/no-logs render for runs
        whose trigger.dev task is `platos-agent-batch`. The batch view reads
        the run's `metadata` column directly, so trace presence doesn't
        matter (the generic trace viewer is still available from other
        surfaces if ever needed).
      */}
      {run.taskIdentifier === BATCH_TASK_IDENTIFIER ? (
        <BatchRunView run={run} />
      ) : (
...
/**
 * K.3 — NavBar breadcrumb that jumps up from a run to its parent agent +
 * thread. Reads `agentId`/`threadId` off the run's JSON-serialised metadata
 * (same blob the agent runtime writes on spawn; see `agent-tool-block.task.ts`
 * + `agent-batch.task.ts`). Renders nothing for non-agent runs so ordinary
 * trigger.dev runs keep their clean NavBar. The agent + thread links jump to
 * the scoped webapp routes via the existing pathBuilder helpers.
 */
function RunAgentBreadcrumb({ run }: { run: LoaderData["run"] }) {
  const organization = useOrganization();
  const project = useProject();
  const environment = useEnvironment();

  const parsed = parseAgentMetadata(run.metadata ?? null);
  if (!parsed) return null;

  const agentHref = agentPath(organization, project, environment, parsed.agentId);
  const threadHref = parsed.threadId
    ? agentConversationPath(organization, project, environment, parsed.agentId, parsed.threadId)
    : null;

  return (
    <nav aria-label="Run context" className="flex min-w-0 items-center gap-1 text-xs">
      <LinkButton variant="minimal/small" to={agentHref}>
        Agent
      </LinkButton>
      <span className="text-text-dimmed">/</span>
      {threadHref ? (
        <LinkButton variant="minimal/small" to={threadHref}>
          Thread
        </LinkButton>
      ) : (
        <span className="px-1.5 text-text-dimmed">Thread</span>
      )}
      <span className="text-text-dimmed">/</span>
      <span className="px-1.5 font-mono text-text-bright" title={run.friendlyId}>
        Run {run.friendlyId}
      </span>
    </nav>
  );
