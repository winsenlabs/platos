      ...
      try {
        ...
        for await (const part of result.fullStream) {
          switch (part.type) {
            ...
            // PRELAUNCH-A2-20 — `finish` was previously handled implicitly
            // via stream end; emit a no-op so the switch is exhaustive
            // and v7 (which may stop emitting some implicit finish frames)
            // doesn't regress us. Plus minimal handlers for v6 chunk
            // types this route doesn't surface.
            case "finish":
...
function buildSystemPrompt(
  ...
  let prompt = `You are a JSON payload generator for a Platos task with id "${taskIdentifier}".
