...
export async function action({ request, params }: ActionFunctionArgs) {
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        // PRELAUNCH-A2-17 — propagate the request's abort signal so a
        // client disconnect cancels the upstream LLM call (no shadow
        // billing on abandoned generations).
        ...
        for await (const part of result.fullStream) {
          switch (part.type) {
            ...
            // PRELAUNCH-A2-19 — silently consume the v6 chunk types this
            // route doesn't render. The query-generator UX only needs
            // `text-delta` + `tool-call` + `finish` + `error`; surfacing
            // reasoning / tool-result / source / file / abort / start /
            // finish-step / raw is up to a future iteration.
