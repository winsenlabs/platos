...
export default function Project() {
  ...
  return (
    <div className="grid min-h-0 grid-cols-1 overflow-hidden sm:grid-cols-[15rem_1fr]">
      <aside className="hidden min-h-0 flex-col border-r border-grid-bright bg-background-dimmed p-3 sm:flex">
        <div className="mb-4">
      ...
      <div className="flex min-h-0 min-w-0 flex-col">
        <div className="flex shrink-0 items-center gap-3 border-b border-grid-bright bg-background-dimmed px-3 py-2 sm:hidden">
          <div className="min-w-0 flex-1">
            <p className="truncate text-sm font-semibold text-text-bright">{organization.name}</p>
            <p className="truncate text-xs text-text-dimmed">{project.name}</p>
          </div>
          <nav aria-label="Scope navigation" className="flex max-w-[60%] gap-1 overflow-x-auto">
            {project.environments.map((item) => (
              <Link
                key={item.id}
                to={v3EnvironmentPath(organization, project, item)}
