...
export const meta: MetaFunction = () => {
  return [{ title: `Add Private Connection | Platos` }];
};
...
function TerraformWizard({ awsAccountIds }: { awsAccountIds: string[] }) {
  ...
  const terraformScript = `# Platos Private Networking - Terraform Configuration
...
output "endpoint_service_name" {
  description = "Paste this into the Platos dashboard"
...
function AIPromptWizard({ awsAccountIds }: { awsAccountIds: string[] }) {
  ...
  const prompt = `I need to set up AWS PrivateLink so that Platos can connect to my resource. Please create the following in my AWS account in the ${region} (${regionLabel}) region:
...
3. A VPC Endpoint Service:
   ...
   - Allowed principals:
${awsAccountIds.map((id) => `     - arn:aws:iam::${id}:root`).join("\n") || "     - <Platos AWS account ARN>"}

After creating everything, give me the VPC Endpoint Service name (it looks like com.amazonaws.vpce.<region>.vpce-svc-*) so I can paste it into the Platos dashboard.`;
          ...
          <div>
            <div className="mb-4 border-b border-grid-dimmed pb-3">
              ...
              <Paragraph variant="small">
                Connect your AWS resources to Platos task pods via AWS PrivateLink. You'll need
            ...
            {setupMethod === "docs" && (
              <div className="mb-6 rounded-lg border border-grid-dimmed p-4">
                ...
                {/* TODO(Theme-P): replace with docs.platos.dev/network/private-link-setup-guide once docs site ships */}
                <iframe
                  src="https://github.com/platos-dev/platos#private-link-setup-guide"
