...
export default function AgentSkillsRoute() {
  ...
  return (
    <Page>
      ...
      {fetcher.data && <pre className={`mt-4 rounded border p-3 text-xs ${fetcher.data.ok ? "border-grid-bright" : "border-red-500/40 text-red-300"}`}>{stableJson(fetcher.data)}</pre>}
    </Page>
  );
}
