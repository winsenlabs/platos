...
export default function Page() {
  ...
  const [form, { environmentIds, variables }] = useForm({
    ...
    defaultValue: {
      // PPR-63 — pre-fill first row key when deep-linked via `?key=FOO`.
      variables: [{ key: prefillKey ?? "", value: "" }],
    },
  });
...
function VariableFields({
  ...
  /** PPR-63 — pre-fill the first row's key when deep-linked via `?key=FOO`. */
  initialKey?: string;
}) {
  const {
    items,
    ...
    delete: remove,
    insertAfter,
  } = useList<Variable>([{ key: initialKey ?? "", value: "" }]);
