...
import { GitMeta, tryCatch } from "@platos/core/v3";
...
// OSS launch: Preview branches is gated on the upstream billing/platform
// stack (currentPlan, addOns, branch-purchase pricing) that does not ship
// with Platos OSS. Rather than render a partially-broken upgrade flow that
// links to a billing page that doesn't exist, the loader short-circuits
// and the Page renders a "Coming soon" placeholder. The full v3 UI stays
// in source under git history and can be re-enabled once the OSS billing
// surface lands. The action is left intact so resources.branches.archive
// (and any other path that imports the action types) keeps compiling.
const SHOW_PREVIEW_BRANCHES_OSS = false;

export const loader = async ({ request, params }: LoaderFunctionArgs) => {
  const userId = await requireUserId(request);

  if (!SHOW_PREVIEW_BRANCHES_OSS) {
    return typedjson({ ossPlaceholder: true as const });
  }
  ...
  try {
    ...
    return typedjson({ ossPlaceholder: false as const, ...result });
  } catch (error) {
...
export default function Page() {
  ...
  return data.ossPlaceholder ? <OssComingSoon /> : <BranchesPage data={data} />;
}

function OssComingSoon() {
  return (
    <PageContainer>
      <NavBar>
      ...
      <PageBody>
        <MainCenteredContainer className="max-w-xl">
          <div className="rounded-md border border-grid-bright bg-background-bright p-6">
            <Header3 spacing>Preview branches — coming soon</Header3>
            <Paragraph spacing variant="small">
              Per-branch preview environments are not yet wired up in the open-source
              distribution. The schema and engine support is already in place; the
              dashboard surface will land in a follow-up release alongside the
              self-serve preview-deployment flow.
            </Paragraph>
            <Paragraph variant="small" className="text-text-dimmed">
              Until then, use one of the built-in environments (dev / staging /
              production) for your runs. Branches you create directly via the{" "}
              <InlineCode variant="extra-small">RuntimeEnvironment</InlineCode> table
              continue to work — only the dashboard view is gated.
            </Paragraph>
          </div>
...
type BranchesPageData = Extract<
  ReturnType<typeof useTypedLoaderData<typeof loader>>,
  { ossPlaceholder: false }
>;

function BranchesPage({ data }: { data: BranchesPageData }) {
  const {
