...
import { inviteMembers, MemberLimitReachedError } from "~/models/member.server";
  ...
  try {
    ...
    if (error instanceof MemberLimitReachedError) {
      return json(
        {
          errors: {
            body: `OSS member limit reached (max ${error.limit} per org). Self-host without limits by raising PLATOS_MAX_PROJECT_MEMBERS.`,
          },
        },
        { status: 400 }
      );
...
export default function Page() {
  ...
  const submissionError =
    ...
    "errors" in lastSubmission &&
    lastSubmission.errors &&
    typeof lastSubmission.errors === "object" &&
    "body" in lastSubmission.errors
      ? String(lastSubmission.errors.body)
      : null;
    ...
    <MainCenteredContainer className="max-w-[26rem] rounded-lg border border-grid-bright bg-background-dimmed p-5 shadow-lg">
      <div>
        ...
        <Form method="post" {...form.props}>
          <Fieldset>
            {submissionError ? (
              <FormError className="mb-2">{submissionError}</FormError>
            ) : null}
            <InputGroup>
