import { millisecondsToNanoseconds } from "@platos/core/v3";
  ...
  public async call({
    ...
    const run = await this.#prismaClient.taskRun.findFirstOrThrow({
      select: {
        ...
        completedAt: true,
        logsDeletedAt: true,
        // W.3 — batch viewer needs metadata + cost to render the per-item grid
        // and summary card. `metadata` is the JSON-serialised blob populated by
        // the agent-batch task via `metadata.set("progress", ...)`.
        metadata: true,
        metadataType: true,
        costInCents: true,
    ...
    const runData = {
      ...
      // W.3 — surface these so the route can detect the batch task and render
      // the batch viewer in place of the default trace view.
