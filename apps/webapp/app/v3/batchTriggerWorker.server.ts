...
/**
 * Batch trigger worker for processing RunEngine batches.
 *
 * NOTE: Run Engine v2 inline batches (batchVersion: "runengine:v2") use the new BatchQueue
 * system with Deficit Round Robin scheduling, which is encapsulated within the RunEngine.
 * See runEngine.server.ts for the configuration. This worker handles the legacy "runengine:v1"
 * batchVersion only.
 */
