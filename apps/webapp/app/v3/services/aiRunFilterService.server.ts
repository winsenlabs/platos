...
export class AIRunFilterService {
  ...
  async call(
    ...
    try {
      const result = await generateText({
        model: this.model,
        // PRELAUNCH-A2-21 — `experimental_output` is deprecated under v6
        // and slated for removal in v7. Migrate to the stable `output` API.
        output: Output.object({ schema: AIFilterResponseSchema }),
        // PRELAUNCH-A2-16 — propagate caller's abort signal.
  ...
  `,
      ...
      const validationResult = AIFilters.safeParse(result.output.filters);
