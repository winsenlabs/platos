...
export interface AIQueryOptions {
  ...
  currentQuery?: string;
  /**
   * PRELAUNCH-A2-14 — caller's abort signal. Threaded through to the
   * underlying streamText / generateText call so a client disconnect
   * (Remix loader's `request.signal`) cancels the upstream LLM call.
   */
...
export class AIQueryService {
  ...
  streamQuery(prompt: string, options: AIQueryOptions = {}) {
    const { mode = "new", currentQuery, abortSignal } = options;
    ...
    return streamText({
      ...
      // PRELAUNCH-A2-14 — propagate caller's abort signal.
      abortSignal,
  ...
  async call(prompt: string, options: AIQueryOptions = {}): Promise<AIQueryResult> {
    ...
    const result = await streamText({
      ...
      stopWhen: stepCountIs(5),
      // PRELAUNCH-A2-14 — propagate caller's abort signal (non-streaming
      // call-site at :194).
      abortSignal,
      experimental_telemetry: {
        isEnabled: true,
