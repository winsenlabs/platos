...
export const META_TOOL_CATEGORIES: Record<string, string> = {
  ...
  // artifacts — Theme F
  generate_artifact: "artifacts",
  revise_artifact: "artifacts",
  // profile — per-user KV
...
export class AgentService {
  ...
  async getAgentConfig(
    ...
    const defaults: AgentConfig = {
      ...
      metaTools: {
        ...
        // Theme F.6 — artifact meta-tools enabled by default. Always
        // scope-checked + kind-validated; safe to leave on.
  ...
  private buildMetaTools(
    ...
    // ────────────────────────────────────────────────────────────────
    // Theme F.6 — artifact meta-tools (generate_artifact, revise_artifact).
    //
    // `generate_artifact` stores a new PlatosAgentArtifact row keyed by a
    // freshly-minted `artifactKey`, revision 1. `revise_artifact` looks up
    // the latest revision of an existing `artifactKey` within the current
    // thread + scope, then appends a new row (`revision = prior + 1`) —
    // previous rows are never mutated. The meta-tools hard-require a
    // `threadId` on the scope (set via `scope.sessionId` by the task
    // runner) so cross-thread / cross-scope writes fail closed.
    //
    // Kind + content validation lives in artifact-meta.ts so the pure
    // logic is unit-tested independently of the Nest / Prisma harness.
    // ────────────────────────────────────────────────────────────────
    const resolveThreadId = (): string | null => {
      const anyScope = scope as any;
      return anyScope?.threadId || anyScope?.sessionId || null;
    };

    tools.generate_artifact = {
      description:
        "Create a new artifact (markdown | code | html | json | csv | svg | image) attached to the current conversation thread. Returns { artifactId, artifactKey, revision } which the caller can later pass to `revise_artifact` to version the content without losing history. Use this when producing a document, a code sample, an HTML widget, a chart (SVG), or structured data worth preserving outside the chat stream.",
      inputSchema: z.object({
        type: z
          .enum(ARTIFACT_KINDS as unknown as [ArtifactKind, ...ArtifactKind[]])
          .describe("Canonical artifact kind. Must be one of: " + ARTIFACT_KINDS.join(", ")),
        content: z.string().min(1).describe("Artifact body. Validated against the kind — HTML rejects inline <script> + on* handlers, JSON must parse, SVG must start with <svg>, image must be a platos-attachment:// reference or a data:image/* URL."),
        title: z.string().optional().describe("Human-readable title (e.g., 'Q1 Sales Summary')"),
        language: z
          .string()
          .optional()
          .describe("Language tag for code artifacts (e.g., 'typescript', 'python'). Ignored for non-code kinds."),
        metadata: z
          .record(z.unknown())
          .optional()
          .describe("Free-form rendering hints / source metadata."),
      }),
      execute: async ({ type, content, title, language, metadata }) => {
        // Theme F.7 — streaming events are emitted at each milestone. All
        // emissions are wrapped in try/catch so a broken sink can never
        ...
        const emit = (event: AgentStreamEvent) => {
          ...
          try {
            ...
            this.logger.warn(
              `[artifact.generate] stream-sink threw — dropping event type=${event.type}: ${(sinkErr as any)?.message ?? String(sinkErr)}`,
            );
          }
        };
        // Provisional key generated up-front so `artifact_start` /
        // `artifact_delta` can reference it even before the DB row lands.
        const artifactKey = newArtifactKey();
        try {
          ...
          if (!threadId) {
            emit({
              ...
              artifactKey,
              code: "scope_mismatch",
              message: "No thread context — generate_artifact requires an active thread",
            });
            return { status: "failed", error: "No thread context — generate_artifact requires an active thread" };
          }
          let kind: ArtifactKind;
          try {
            kind = validateKind(type);
          } catch (kindErr: any) {
            emit({
              type: "artifact_error",
              artifactKey,
              code: "invalid_kind",
              message: kindErr?.message || "Invalid artifact kind",
            });
            return { status: "failed", error: kindErr?.message || "Invalid artifact kind" };
          }
          try {
            validateContentForKind(kind, content);
          } catch (contentErr: any) {
            emit({
              type: "artifact_error",
              artifactKey,
              code: "invalid_content",
              message: contentErr?.message || "Invalid artifact content",
            });
            return { status: "failed", error: contentErr?.message || "Invalid artifact content" };
          }

          // Scope-check the thread: the thread must belong to the current
          // (org, project, env). Cross-scope writes fail closed.
          const thread = await this.prisma.platosAgentThread.findFirst({
            where: {
              id: threadId,
              organizationId: scope.organizationId,
          ...
          if (!thread) {
            emit({
              ...
              artifactKey,
              code: "scope_mismatch",
              message: "Thread not found in current scope — refusing to write artifact",
            });
            return {
              status: "failed",
              error: "Thread not found in current scope — refusing to write artifact",
            };
          }

          // Emit `artifact_start` before the insert. The UI can use the
          // provisional artifactKey + provisional artifactId (= artifactKey
          // until the row commits) to open a placeholder bubble.
          emit({
            type: "artifact_start",
          ...
          // Meta-tool writes the whole content in one shot, so emit a
          // single-chunk delta carrying the full body. A future follow-up
          // can switch to true incremental deltas when we stream artifact
          // content through the model instead of through `execute()`.
          emit({
            type: "artifact_delta",
            artifactId: artifactKey,
            artifactKey,
          ...
          let row;
          try {
            row = await this.prisma.platosAgentArtifact.create({
              data: {
                organizationId: scope.organizationId,
                projectId: scope.projectId,
                environmentId: scope.environmentId,
                ...
                createdBy: `agent:${scope.agentId || "default"}`,
              },
              select: { id: true, artifactKey: true, revision: true, kind: true, title: true, language: true, createdAt: true },
            });
          } catch (dbErr: any) {
            // Short-circuit via return (not rethrow) so the outer catch
            // doesn't double-emit a persist_failed event.
            emit({
              type: "artifact_error",
              artifactKey,
              code: "persist_failed",
              message: dbErr?.message || "generate_artifact DB write failed",
            });
            return {
              status: "failed",
              error: dbErr?.message || "generate_artifact DB write failed",
            };
          }
          this.logger.log(
            `[artifact.generate] thread=${threadId} artifactKey=${row.artifactKey} kind=${row.kind} rev=${row.revision}`,
          );
          ...
          // Errors after the pre-start validations that weren't already
          // surfaced as artifact_error events (e.g. Prisma blew up after
          // we'd already emitted persist_failed) — fall back to a generic
          // persist_failed so the UI always terminates the placeholder.
          emit({
            type: "artifact_error",
            artifactKey,
            code: "persist_failed",
            message: err?.message || "generate_artifact failed",
          });
          return { status: "failed", error: err?.message || "generate_artifact failed" };
        }
      },
    };

    tools.revise_artifact = {
      description:
        "Append a new revision to an existing artifact. Identify the artifact by EITHER `artifactId` (a specific revision row's id) OR `artifactKey` (the stable grouping key). Kind is carried forward from the prior revision — you cannot change kind mid-artifact. Returns { artifactId, artifactKey, revision } for the newly-appended row. Previous revisions are preserved (immutable history).",
      inputSchema: z.object({
        artifactId: z
          .string()
          .optional()
          .describe("Any prior revision's id. Use this when you have the id from `generate_artifact`."),
        artifactKey: z
          .string()
          .optional()
          .describe("Alternative: the stable key shared across revisions. Use this when you already have the key."),
        newContent: z.string().min(1).describe("New content for this revision. Validated against the artifact's kind."),
        reason: z.string().optional().describe("Why this revision was made — recorded in metadata for audit."),
        title: z.string().optional().describe("Optional updated title for this revision."),
        metadata: z.record(z.unknown()).optional().describe("Optional extra metadata merged into the row."),
      }),
      execute: async ({ artifactId, artifactKey, newContent, reason, title, metadata }) => {
        // Theme F.7 — stream artifact lifecycle events. See note in
        // `generate_artifact` above for the sink contract.
        const emit = (event: AgentStreamEvent) => {
          if (!onArtifactEvent) return;
          try {
            onArtifactEvent(event);
            ...
            this.logger.warn(
              `[artifact.revise] stream-sink threw — dropping event type=${event.type}: ${(sinkErr as any)?.message ?? String(sinkErr)}`,
            );
          }
        };
        try {
          ...
          if (!threadId) {
            emit({
              ...
              code: "scope_mismatch",
              message: "No thread context — revise_artifact requires an active thread",
            });
            return { status: "failed", error: "No thread context — revise_artifact requires an active thread" };
          }
          if (!artifactId && !artifactKey) {
            emit({
              type: "artifact_error",
              code: "not_found",
              message: "Must supply either artifactId or artifactKey",
            });
            return { status: "failed", error: "Must supply either artifactId or artifactKey" };
          }

          // Look up the latest revision of the target artifact, scoped to
          // the current (org, project, env) + thread. Cross-scope reads
          // return null so the write path fails closed.
          const scopedWhere = {
            organizationId: scope.organizationId,
            projectId: scope.projectId,
            environmentId: scope.environmentId,
          ...
          let latest: { id: string; artifactKey: string; revision: number; kind: string } | null = null;
          if (artifactKey) {
            latest = await this.prisma.platosAgentArtifact.findFirst({
              where: { ...scopedWhere, artifactKey },
              orderBy: { revision: "desc" },
              select: { id: true, artifactKey: true, revision: true, kind: true },
            });
          } else {
            // Resolve the row from artifactId first, then re-select the latest
            // revision of that artifactKey so we always append on top.
            const anchor = await this.prisma.platosAgentArtifact.findFirst({
              where: { ...scopedWhere, id: artifactId! },
              select: { artifactKey: true },
            });
            if (anchor?.artifactKey) {
              latest = await this.prisma.platosAgentArtifact.findFirst({
                where: { ...scopedWhere, artifactKey: anchor.artifactKey },
                orderBy: { revision: "desc" },
                select: { id: true, artifactKey: true, revision: true, kind: true },
              });
            }
          ...
          if (!latest) {
            emit({
              ...
              artifactKey: artifactKey ?? undefined,
              code: "not_found",
              message: "Artifact not found in current scope + thread",
            });
            return { status: "failed", error: "Artifact not found in current scope + thread" };
          }

          let kind: ArtifactKind;
          try {
            kind = validateKind(latest.kind);
          } catch (kindErr: any) {
            emit({
              type: "artifact_error",
          ...
          try {
            validateContentForKind(kind, newContent);
          } catch (contentErr: any) {
            emit({
              type: "artifact_error",
              artifactId: latest.id,
          ...
          // Emit `artifact_start` carrying the resolved latest-revision id
          // so the UI can pin the stream to this existing artifact (instead
          // of opening a new bubble).
          emit({
            type: "artifact_start",
            artifactId: latest.id,
            artifactKey: latest.artifactKey,
          ...
          // Single-chunk delta — see note in `generate_artifact`.
          emit({
            type: "artifact_delta",
            artifactId: latest.id,
            artifactKey: latest.artifactKey,
          ...
          const mergedMetadata = {
            ...
            prior_revision_id: latest.id,
          };

          let next;
          try {
            next = await this.prisma.platosAgentArtifact.create({
              data: {
                organizationId: scope.organizationId,
                projectId: scope.projectId,
                environmentId: scope.environmentId,
            ...
            // Emit the persist_failed event here with the correct ids —
            // the outer catch only knows about the original arg ids. We
            // short-circuit via return instead of rethrowing so the outer
            // catch doesn't double-emit.
            emit({
              type: "artifact_error",
              artifactId: latest.id,
              artifactKey: latest.artifactKey,
              code: "persist_failed",
              message: dbErr?.message || "revise_artifact DB write failed",
            });
            return {
              status: "failed",
              error: dbErr?.message || "revise_artifact DB write failed",
            };
          }
          this.logger.log(
            `[artifact.revise] thread=${threadId} artifactKey=${next.artifactKey} kind=${next.kind} rev=${next.revision} (prior=${latest.id})`,
          );

          emit({
            type: "artifact_committed",
            artifactId: next.id,
            artifactKey: next.artifactKey,
            kind: next.kind as ArtifactKind,
          ...
          emit({
            ...
            artifactKey: artifactKey ?? undefined,
            code: "persist_failed",
            message: err?.message || "revise_artifact failed",
          });
          return { status: "failed", error: err?.message || "revise_artifact failed" };
        }
      },
    };
    ...
    if (attachments && attachments.length > 0) {
      ...
      if (native) {
        ...
        for (const a of attachments) {
          if (a.kind === "image") {
            parts.push({ type: "image", image: a.data, mimeType: a.mimeType });
          } else {
            parts.push({ type: "file", data: a.data, mimeType: a.mimeType });
          }
          this.logger.log(
            `[attachments] built ${a.kind} part: id=${a.id} mime=${a.mimeType} bytes=${a.bytes}`,
          );
        }
