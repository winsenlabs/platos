...
configure({
  secretKey: process.env["TRIGGER_ACCESS_TOKEN"], // Platos user tokens start with plt_pat_
});
...
A PAT is a token associated with a specific user, and gives access to all the orgs, projects, and environments that the user has access to. You can identify a Platos PAT by the `plt_pat_` prefix. Because a PAT does not scope access to a specific environment, you must provide the `projectRef` argument when using a PAT (and sometimes the environment as well).
...
configure({
  secretKey: process.env["TRIGGER_ACCESS_TOKEN"], // starts with plt_pat_
});
