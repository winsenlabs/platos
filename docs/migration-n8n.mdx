...
| Loop over N items → Execute Sub-workflow → Merge  | [tasks.batchTriggerAndWait()](/triggering#yourtask-batchtriggerandwait)                                            |
...
| Retry on Fail (per-node setting)                  | [retry.maxAttempts](/tasks/overview#retry-options) (retries the whole `run()`, not a single step)                  |
