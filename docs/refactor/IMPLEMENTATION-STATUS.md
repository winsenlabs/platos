...
| 🟡 **Webapp — coupled/decision routes remaining**                                         | `regions/limits/concurrency/branches` (coupled to shared components, need untangle) + the **`runs`** dashboard (52 files + `components/runs`, awaiting the strip-vs-repurpose call). KEEP: run-engine api.v1._/engine._, environment-variables (Platos scoped secrets), apikeys, settings.integrations (Platos `.mcp`). |
...
| ✅ Delete the unused vendored Trigger CLI                                  | Removed in WIN-162; use Trigger's published CLI instead.                                          |
...
| 🔒 Retire/repoint `apps/webapp` run-engine role + `worker` compose service | The webapp _is_ the forked platform; needs the standalone-console decision (§13).                 |
