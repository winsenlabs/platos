...
| New trigger tasks: `durable-turn`, `agent-session`, `approval-waitpoint`, `employee-run`, `skill-run` | ⏳ | Scaffolds under `apps/agent/src/trigger-tasks/`, existing `@platos/sdk` (SDK swap is separate/staged). |
