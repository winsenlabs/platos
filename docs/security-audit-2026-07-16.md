...
## 0c. Phase 5 batch 3 — M8, L8, L7, M3 (2026-07-17)

All deployed to test.platos, Fable-verified (**no security holes**), durable smoke green.

- **M8 (PII ReDoS) — CLOSED (durable).** Admin `customRegex` now compiles with **RE2** (linear-time, no backtracking) instead of `new RegExp`, closing the alternation-overlap shapes (`(a|ab)*`) the old heuristic missed. The built-in detectors stay on `RegExp`. The `matchAll` loop became an `exec()`/`lastIndex` loop (RE2 has no `matchAll`) with a zero-width-match guard. *Residual (fail-**open** edge, admin-facing): RE2 silently applies the `u` flag, so a `customRegex` that was valid under non-`u` `RegExp` but invalid under `u` (e.g. an unnecessary escape) now throws at construction → that one filter is skipped (logged at warn). DoS-safe but the PII rule silently stops applying — surface to admins.* **PACKAGING GOTCHA that took test.platos down for one build cycle:** `re2` is a native addon; pnpm v7+ won't run its build script (which compiles `re2.node`) unless it's in root `package.json` `pnpm.onlyBuiltDependencies`. `tsc` passed green, then the agent crash-looped at boot on `Cannot find module './build/Release/re2.node'`. Fix = add `re2` to that allowlist. Any future native addon needs the same.
- **L8 (body-cap DoS) — CLOSED.** A Content-Length guard registered BEFORE `useBodyParser` caps the unauthenticated bypass surface: `/oauth` + `/api/v1/public` at 256KB, `/mcp` at 2MB (env-tunable `PLATOS_MCP_BODY_CAP_BYTES`; raised from the audit's flat 256KB because MCP tool calls can legitimately carry document-sized args). The authenticated 15mb catalog route is unaffected. *Tradeoff: chunked / no-Content-Length requests on those prefixes are rejected; MCP ingest >2MB now 413s pre-auth (raise the env var if needed).*
- **L7 (unscoped run inspect/replay) — CLOSED.** `get_run_details` and `replay_run` now retrieve the run and reject unless its trigger-time `metadata.{organizationId,projectId,environmentId}` matches the caller's scope (fail-closed). All THREE `platos-custom-task` trigger sites — agent dispatch, dashboard "Run" (`platos-tasks.controller.ts`), and the `platos_tasks` MCP tool — now stamp scope tags+metadata (Fable caught that the two manual sites were untagged, which would have denied a run to its own owner). *Hardening residual: the check reads `metadata`, which a task body that runtime-`metadata.set`s from an agent-controlled payload could overwrite; verifying against the immutable trigger-time `tags` instead would be more robust (low impact — needs a cuid runId guess + a self-overwriting task).*
- **M3 (re-encrypt sweep) — SHIPPED as an operational script** (`scripts/reencrypt-entity-oauth-tokens.ts`, NOT deployed code). One-off `tsx` sweep re-encrypting legacy message-key / plaintext entity OAuth tokens onto `PLATOS_ENCRYPTION_KEY` so the dual-read can eventually be deleted. Crux solved without a schema marker: the **AES-256-GCM auth tag IS the marker** — trial-decrypt in key priority order; a value that's pure base64 ≥32 bytes yet neither key authenticates is reported AMBIGUOUS and never written (exit 2). Dry-run by default. Crypto is byte-identical to `SecretsService` (Fable-verified: cannot corrupt or double-wrap). *Operator runs it against the DB when ready.*

## 0b. Phase 5 — residual close-out (2026-07-17)

Branch `security-hardening-p5`. Opus authored the patches, Fable adversarially verified (it verified the undici pin against undici's own source, and found a **live SSRF on `main`** — see H11 below). All deployed to test.platos, VPS-typecheck-green.

- **H11 (DNS-rebind TOCTOU) — CLOSED.** `fetchWithValidatedRedirects` now pins the validated IPs into the socket via a per-call undici `Agent` with a `connect.lookup` that returns only already-validated addresses and fails closed for any unpinned host (native Node `fetch` is undici on node:22-slim, so an `http.Agent` lookup would have been inert — the fix imports `fetch` from undici so dispatcher-honoring is structural). Added the `undici` dep. **Also fixed a live SSRF Fable found on `main`:** `URL.hostname` keeps IPv6 literals bracketed (`[::ffff:169.254.169.254]`), so `net.isIP` returned 0, the literal-IP rejection was skipped, DNS on the bracketed string failed silently, and the URL passed validation as "public, unresolvable" → a plain `fetch` (which strips brackets) connected to IMDS. Fixed with `unbracketHost()` before every IP test — this protects **all** `validatePublicUrl` callers. The two webhook plain-`fetch` paths (`events.service.ts`, `budget-alert.task.ts`) were also converted to the pinning fetch. *Residual: privileged headers can still follow a redirect to an arbitrary PUBLIC host — SSRF-to-internal is closed, exfil-to-public is a separate, lower item.*
- **H12 (dup-`toolName` ordering) — CLOSED.** Both `tools/list` and `tools/call` now load ALL exposed ACL rows for a name and admit only if EVERY row admits the caller (most-restrictive wins) — order-independent, so no `orderBy` and the two paths agree by construction. *Follow-up: a `@@unique([entityPk, toolName])` migration (after de-dup) would let both go back to a single-row read.*
- **L5 (RAG injection) — injection half CLOSED.** The automatic memory injection now passes `excludeRag: true`, filtering on the plaintext `source = "rag"` column (a `metadata->>'__rag'` predicate would fail OPEN — `metadata` is envelope-encrypted at rest). RAG ingest now stamps `agentId` + `source:"rag"`; the exclusion and the stamp ship together (reverting one silently re-opens the prompt-budget bug). The `recall` meta-tool (both single-agent + cluster branches) also excludes RAG so it returns memories, not document chunks; `rag_retrieve` still finds every chunk (old NULL-agentId and new alike). *Residuals: (1) `rag_retrieve` is still not agent-scoped — closing it would strand old NULL-agentId chunks, needs a backfill + cluster-semantics decision; **L5 is NOT fully closed.** (2) A GDPR export→import round-trip rewrites `source` back to `"imported"`, defeating the discriminator for re-imported RAG chunks — preserve `source` on import, or JS-post-filter `__rag` in the injection path as belt-and-braces. (3) Batch RAG ingest now runs the child as the real acting agent instead of `"default"` — intended provenance, smoke-test once.*
...
> **M2** — `session-token.controller.ts` mint spread `body.claims` LAST (caller could override tenancy/identity/agent-pin → cross-org mint on entityId slug collision). Now a `RESERVED_CLAIM_KEYS` set strips every verifier-trusted key and the sanitized bag is spread FIRST (typed fields win). **M3** — `oauth.controller.ts` entity OAuth tokens were encrypted with the message-body key and **failed open to plaintext**; now routed through `SecretsService` (`PLATOS_ENCRYPTION_KEY`, fail-closed in prod), no plaintext fallback, with a `legacyDecryptEntityTokenWithMessageKey` dual-read for old rows (delete after a re-encrypt window). **M5** — `tool-sync-ws.service.ts` `PendingCall` now records `{entityId, environmentId}` at dispatch; the `tool_result`/`tool_error` handlers reject a frame from a non-matching responder socket (return early WITHOUT consuming the pending entry), closing cross-tenant forged-result/abort injection. **M8** — `pii-filter.service.ts` admin `customRegex` now gated by `isSafeCustomRegex` (512-char cap + nested-quantifier heuristic) before compile, plus a 50k input cap.
>
> **Fable cross-model verify:** M2/M3/M5 **CLOSED** (reserved-set complete incl. `isGuest`; no prototype-pollution smuggle; M3 wire-format matches legacy so old rows fall through to the dual-read cleanly, no uncaught throw; M5 identity is the authenticated connect-time entity, check correctly placed pre-`clearTimeout`, no DoS). **Regression caught + fixed:** M2 stripped `userMeta`, the only channel for `{{user.name}}`/`{{user.email}}` prompt hints through this endpoint — restored as a **typed** `userMeta` body field (controlled path, still out of the injectable bag). **M8 CLOSED with a documented RESIDUAL (deferred to Phase 4):** the nested-quantifier heuristic misses alternation-overlap shapes (`(a|ab)*`, `(a|a)*`, `((a)*)*`) and the 50k cap bounds polynomial but not exponential blowup — `customRegex` is privileged per-agent governance config, but the impact is cross-tenant (shared event loop). **Durable fix = run custom patterns through RE2** (native linear-time engine; needs an `re2` dep + native build) — tracked as a Phase 4 item, plus config-write-time validation on the governance update endpoint.
...
> **STATUS — DESIGNED, verdict NEEDS-HUMAN (2026-07-17). Full design + migrations + 5-step rollout: `docs/security-rls-design.md`. NOT applied.**
> Two blockers make the audit's "just add `$extends` + policies" plan silently do NOTHING as deployed, and make auto-applying the enforcing step **break prod to all-empty rows** — so this is human-sequenced, not loop-applied:
> 1. **The app connects as the Postgres SUPERUSER** (`POSTGRES_USER=postgres` in docker-compose/.env). Superusers bypass RLS unconditionally; `FORCE ROW LEVEL SECURITY` doesn't change that. RLS is 100% inert until `DATABASE_URL` is cut over to a dedicated non-superuser role — an **infra** change. The enforcing migration would pass CI and protect nothing.
> 2. **No per-request scope context** for a `$extends` to read — zero AsyncLocalStorage/CLS in the agent; the Prisma client is one global singleton used by 53 files. An ALS layer wired from ScopeGuard + WS handshake + every trigger task must exist first.
> Also: the trigger task news up its own `PrismaClient` (no wrapper → all-empty under enforcing), the webapp main+replica clients, and the admin cross-scope sweeps (`audit.cross_scope_tool_calls`, `budgets.rollup_org_wide`, `gdpr.export_user_everywhere`) would return **partial data silently** unless run as a `BYPASSRLS` role. `PlatosToolDefinition` is a global catalog with no scope columns — blanket RLS would empty it (the migration correctly skips no-tuple tables via an `information_schema` loop). The connection-GUC mechanic IS solvable (no pgbouncer; `SET LOCAL` visibility intact via array-`$transaction`). The **log-only** migration is genuinely zero-risk to apply (inert under superuser; PERMISSIVE always-true, only `RAISE WARNING`) but zero-*value* until the role cutover. Rollback is a config flip (revert `DATABASE_URL` to superuser).
