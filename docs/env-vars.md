...
| `PLATOS_CHANNELS_WHATSAPP_APP_SECRET` | — | anchor | The Meta app secret (App Dashboard, Settings → Basic). The `X-Hub-Signature-256` on every inbound webhook is the HMAC-SHA256 of the RAW body under this key. Minimum 32 characters. It is a real secret, unlike Discord's public key: anybody holding it can FORGE a delivery. No replay window sits beside it — Meta signs no timestamp, and a window over the customer's send time would refuse the redeliveries an outage caused; a repeat is absorbed by the inbox's idempotency on the message id instead. |
| `PLATOS_CHANNELS_WHATSAPP_VERIFY_TOKEN` | — | with anchor | The token you give Meta when you save the webhook URL; Meta's subscription GET is answered against it and no webhook is ever delivered until that succeeds. Minimum 32 characters. A DIFFERENT secret from the app secret, deliberately. |
| `PLATOS_CHANNELS_TELEGRAM_SECRET_TOKEN` | — | anchor | The `secret_token` you passed to `setWebhook`; Telegram echoes it in `X-Telegram-Bot-Api-Secret-Token` on every update and it is compared in constant time. 32 to 256 characters from `setWebhook`'s own alphabet (`A-Z a-z 0-9 _ -`) — the vendor permits one character, and this refuses anything under 32 because Telegram signs NOTHING: a caller who learns this string can post any body it likes, so its length is the only thing standing in the way. No replay window sits beside it, for a sharper reason than WhatsApp's: with nothing signed, a window would bound an unauthenticated claim. |
