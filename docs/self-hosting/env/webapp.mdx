---
title: "Webapp"
description: "Environment variables for an external Trigger webapp container."
sidebarTitle: "Webapp"
mode: "wide"
---

> These variables configure a separately deployed Trigger application. They
> are not Platos webapp variables and must not be added to the Platos compose
> stack.
...
| `MANAGED_WORKER_SECRET`                          | Yes      | —                     | Managed worker secret (≥16 chars). Must match supervisor. `openssl rand -hex 32`. EOBD.52 removed the silent default. |
| **Domains & ports**                              |          |                       |                                                                                                                    |
...
| `WEBAPP_NODE_MAX_OLD_SPACE_SIZE_MB`              | No       | 1536                  | Runtime V8 old-space ceiling in MiB. Startup rejects values above 75% of the effective container limit or leaving less than 512 MiB outside old-space. |
| `WEBAPP_BUILD_MAX_OLD_SPACE_SIZE_MB`             | No       | 1536                  | Build-only heap ceiling. The build refuses to start unless an additional 2048 MiB is currently available; use off-box image builds for constrained hosts. |
| `WEBAPP_BUILD_SOURCEMAPS`                        | No       | false                 | Enable Remix production source maps/Sentry upload. This materially raises peak memory and should only run on an off-box builder with extra measured headroom. |
