...
> This is an **external Trigger deployment**, not the Platos compose stack.
> Platos does not embed these webapp/worker services or share their runtime
> database. Connect Platos only through explicit `TRIGGER_API_URL` and
> `TRIGGER_SECRET_KEY` configuration.
