...
export async function describeMcpSurface(
  ...
  return withOperator(dependencies, query.authorization, async (grant) => {
    const config = await dependencies.repository.findMcpConfig(grant.scope, query.entityId);
...
export async function readToolAudit(
  ...
  return withOperator(dependencies, query.authorization, async (grant) => {
    ...
    const listed = await dependencies.repository.pageAudit(grant.scope, admitted);
