...
export interface AeadCipher {
   ...
   *
   * WHY IT IS A FOURTH METHOD ON THIS PORT RATHER THAN A SECOND PORT.
   * (It was written as the third; `sealHandle`/`openHandle` above landed in the
   * same milestone, so this port now carries four methods and still one key
   * custodian, which is the point the paragraph is making.) The keys
