// Use case: a delivery attempt failed — decide whether to try again.
//
...
// request back on the queue with an incremented attempt and a delayed
// `availableAt`.
//
...
// durable. The observable schedule is identical: same attempt count, same
// backoff, same give-up point.
//
// GIVING UP IS A SUCCESS, NOT AN ERROR. `ok({ kind: "give-up" })` is the
// terminal outcome after three attempts, exactly as the legacy `return` after
...
export async function recordDeliveryFailure(
  ...
  const decision = decideRetry(request.attempt);
