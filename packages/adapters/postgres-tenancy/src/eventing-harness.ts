...
//
// THE TREE IS TWO ORGANIZATIONS AND THREE ENVIRONMENTS, AND THAT IS THE MINIMUM
// rather than a convenience. `scopedWhere` narrows on `environmentId` AND on the
// project AND on the organization above it, so falsifying each conjunct needs a
// different neighbour:
//
...
/** One organization, one project, two environments — plus the raw ids. */
